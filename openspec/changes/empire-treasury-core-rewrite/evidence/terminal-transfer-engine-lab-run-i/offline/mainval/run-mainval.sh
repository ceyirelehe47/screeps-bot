...
cd "$REPO_ROOT"
START_HEAD=9158c496d77f51072f0899ffa01f60487a031c58
FREEZE_BASE=869149dcdd6f2068572354917bf23c52727cf9b6
VALIDATION_HEAD="$(git rev-parse HEAD)"
OUT="$(node -p 'require("node:fs").mkdtempSync(require("node:path").join(require("node:os").tmpdir(), "treasury-lab-run1-"))')"
...
run probe-protection-freeze git diff --exit-code "$START_HEAD" "$VALIDATION_HEAD" -- \
  test/lab/terminal-transfer/probe.test.ts test/lab/terminal-transfer/controlRecord.ts \
  test/lab/terminal-transfer/singleShot.ts test/lab/terminal-transfer/sendGate.ts \
  test/lab/terminal-transfer/observer.ts test/lab/terminal-transfer/labConfig.ts \
...
# 生产 bundle 跨构建含 buildTime（rollup.config.js:43）非确定——不做跨构建相等断言；
# 改为核对三次实验构建未触碰 dist/main.js（§7.1：实验文件不覆盖 dist/main.js）。
node -e 'const f=require("node:fs"),c=require("node:crypto");console.log(c.createHash("sha256").update(f.readFileSync("dist/main.js")).digest("hex"))' > "$OUT/production-bundle-after-build.txt"
...
run lab-run-i-main node scripts/build-treasury-terminal-lab.mjs --mode run-i-main --out "$OUT/lab-run-i-main"
node -e 'const f=require("node:fs"),c=require("node:crypto");console.log(c.createHash("sha256").update(f.readFileSync("dist/main.js")).digest("hex"))' > "$OUT/production-bundle-after-lab-builds.txt"
cmp "$OUT/production-bundle-after-build.txt" "$OUT/production-bundle-after-lab-builds.txt"
