#!/usr/bin/env bash
# Terminal Transfer Engine Lab Prep I · Remediation I——主验证（任务书 §8.2 模板）。
# 本脚本与其日志放在 OUT 之外的 SCRIPT_DIR；OUT 新建后不再整目录删除。
set -euo pipefail
unset DEST
...
OUT="$(node -p 'const fs = require("node:fs"), os = require("node:os"), path = require("node:path");
let dir;
do { dir = path.join(os.tmpdir(), "treasury-lab-r1-" + Math.random().toString(36).slice(2, 10)); } while (fs.existsSync(dir));
fs.mkdirSync(dir, { recursive: true });
dir')"
LOG="$SCRIPT_DIR/mainval-run.log"
: > "$LOG"
