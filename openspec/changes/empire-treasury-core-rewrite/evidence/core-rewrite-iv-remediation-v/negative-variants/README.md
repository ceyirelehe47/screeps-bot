...
| V1 删缺分支拒绝（恢复 IV 的"仅 eventBranch 存在时校验"+ 移除 memorySnapshot 拒绝） | v1-missing-branch-dropped.patch | I07/I08 共 3 条用例（VService：I07 主用例 1 + I08 两用例） | 缺 eventBranch 恢复被放行（toThrow 红——错结论路径重新打开）；memorySnapshot+oracle、普通数组通道同放行 |
