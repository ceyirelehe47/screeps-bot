...
for (const dir of ["lab-observer", "lab-single-shot", "lab-run-i-main"]) {
  ...
  const checks = {
    ...
    embeddedExperimentId: code.includes('"lab-run1-exec-0001"'),
  };
  if (manifest.mode !== "run-i-main") {
    checks.embeddedShardForst = code.includes('"Forst"');
    checks.embeddedSourceId = code.includes('"b0254141a49b92c"');
    checks.embeddedTargetId = code.includes('"c61a4141a4a9fcb"');
    checks.embeddedCap26 = code.includes("maxFeeEnergy: 26");
  } else {
    checks.embeddedTargetTick557 = code.includes("557");
  }
