...
async function calibrate(api,dir,{c=clock,pause=C.pause}={}) {
  const deadline=c.mono()+K.measurementMaxMs,reads=[];
...
function plan(m, current, profile, stage) {
  ...
  return {kind:'tick-window-admission/v1',runId:P.parentRunId,stage,status:requiredMs<=P.wallMs?'TIME_BUDGET_ADMITTED':'TIME_BUDGET_INSUFFICIENT',
    ...
    exposureLimitMs:P.wallMs,slackMs:P.wallMs-requiredMs,
...
function admit(dir,current,profile,stage) {
  ...
  if(r.status!=='TIME_BUDGET_ADMITTED') C.fail('TIME_BUDGET_INSUFFICIENT');
  return r;
}
