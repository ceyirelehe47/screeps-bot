...
test('source-verified runtime and preview emit four reports and receipts accepted by the final collector under a fresh external run',()=>{
 ...
 assert.equal(policy.measurementSourceBaseCommit,'06ffedb7c558e0bc625f4a2ff450c474fb9d6f1c');
 assert.equal(policy.backupBuild.commit,'5cef62c5bce37c00c4fc35f9fc3881e21128b00b');
 assert.notEqual(policy.measurementSourceBaseCommit,policy.backupBuild.commit);
 ...
 assert.ok(watch.accepted.every(report=>report.productionBase===policy.measurementSourceBaseCommit));
