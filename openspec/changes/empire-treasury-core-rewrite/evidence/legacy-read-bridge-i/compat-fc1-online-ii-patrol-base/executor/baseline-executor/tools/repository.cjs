...
function verifyImplementation(repo){
 ...
 if(h!==P.compatBase||m.expectedSourceTree!==P.compatBaseTree||tree(repo,h)!==P.compatBaseTree||
...
function source(repo){
 ...
 return {head:P.compatBase,tree:P.compatBaseTree,base:P.compatBase,paths:Object.keys(m.files).length,sourceModified:false,implementationHead:P.implementationHead};
}
function baseline(refactor,compat,{network=true}={}){
 ...
 if(C.blob(v)!==P.priorAttemptBlob||a.status!=='NOT_DEPLOYED'||a.terminationReason!=='TIME_BUDGET_INSUFFICIENT'||!a.sourceOff||!C.same(a.writeBoundaries,{candidate:0,restore:0}))C.fail('PRIOR_ATTEMPT_NOT_VERIFIED');
...
function profileFor(t){const startTick=Math.ceil((t+150)/100)*100;return {enabled:true,shardName:'shard1',rooms:P.rooms,resources:P.resources,startTick,endTick:startTick+300,intervalTicks:100,minBucket:2000,maxSampleCpu:2,reserveCpu:5,maxLogBytes:16384};}
