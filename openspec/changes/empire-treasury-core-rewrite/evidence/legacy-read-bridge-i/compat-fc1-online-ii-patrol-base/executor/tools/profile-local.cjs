...
function profileLocal(repo){
 ...
 const old=fs.readFileSync(path.join(repo,'test/treasury-compat/fixtures/core-before-read-envelope-xv.ts.txt'),'utf8');
 ...
 for(const w of workloads){const versions=[];let reference;
  for(const [name,source]of [['XIV',old],['XV',current]]){
 ...
 return {status:'READ_XV_LOCAL_REPLAY_VERIFIED',compiler,liveMemoryUsed:false,engineCpuMeasured:false,engineSpikeReproduced:null,exactOnlineTaskRecordsAvailable:false,
