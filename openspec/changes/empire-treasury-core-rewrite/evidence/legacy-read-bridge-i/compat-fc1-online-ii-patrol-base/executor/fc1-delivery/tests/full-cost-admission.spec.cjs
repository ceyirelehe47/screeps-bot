...
if(!assembled){
 ...
 test('pending authorization blocks the runner and direct observe entry before side effects',async()=>{
  ...
  assert.match(guide,/WAITING_FOR_EXPLICIT_AUTHORIZATION/);
  ...
  await assert.rejects(Observe.observe(args),/NEW_ROUND_AUTHORIZATION_REQUIRED/);
