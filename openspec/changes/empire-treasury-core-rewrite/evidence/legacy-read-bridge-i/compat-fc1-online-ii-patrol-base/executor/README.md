...
本目录由固定执行器 `0e1e14f7` 提取后单独修订。原 `screeps-fc1-pending` 和已归档的旧证据保持原样。发布副本当前仅供离线验证；`policy.json` 的 `newRoundAuthorization.status` 仍为 `WAITING_FOR_EXPLICIT_AUTHORIZATION`，`onlineExecutionReady=false`，执行入口会在网络写入与实验 marker 前拒绝。
