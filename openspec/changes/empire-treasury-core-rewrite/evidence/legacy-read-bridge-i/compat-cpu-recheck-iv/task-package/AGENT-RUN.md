# Compat CPU Recheck IV · Loader III Engine Recheck · Agent 任务书
## 完整实现交付；复测已验收加载优化；四点、预算2、唯一候选上传与受控恢复

本轮不是再次实现 Loader III，也不是完整兼容观察0004。唯一实验变化是使用已提交的优化加载器；采样范围、检查点、预算和安全阈值与已成功收尾的CPU Diagnostic II保持一致。比较只用于判断后续优化方向，不要求性能必须改善才能正确保存诊断。
...
- Loader III generated blob: 7c20e7544bdc45bee11b1ee059ac5df2a12f2bf3
- Prior diagnostic evidence: 2d97f0d93a1bf02191bf4712f1201638263e09ea 的固定 diagnostic-verification.json，blob 5112f9cc691bb1637b3c94e5b9cee0da0d9e9bbb；只读历史对照，不复用旧窗口
- Prerequisites: references/prerequisites.json，基线工具会从真实Git对象库核验CPU II与Loader III原验收文件
...
两仓起始必须干净、本地与远端均匹配。不得用旧CPU II工具直接运行新的源码，因为旧工具固定的生成核心和源码基线已失效；只使用本包。CPU I 与 Loader III 不再重应用。缺少固定对象/依赖或发现远端漂移时报告，不能自行换 base。
...
```bash
PKG="D:/code/screeps/incoming/screeps-compat-cpu-recheck-IV-2026-09-12"
REFACTOR="D:/code/screeps/screeps-bot"
COMPAT="D:/code/screeps/screeps-bot-compat-read-i"
WORK="D:/code/screeps/compat-cpu-recheck-IV-execution"
...
四条命令必须各自退出0。固定工具测试为128项（96项继承回归 + 32项本轮新增），以 references/test-contract.json 的文件集合与数量双重绑定；failed/skipped/todo/cancelled 均为0；摘要必须是 CPU_DIAGNOSTIC_TOOL_TESTS_VERIFIED。测试里有真实子进程、loopback假远端、真实读取器源代码与合成读端口，不属于线上CPU证据。

build 顺序：先检查已提交生成器的 --check 输出，随后 tsconfig.build.json、tsconfig.json 的 noEmit；原版 verify-jest-budget；Rollup build-only。要求195 suites / 685 tests，不能改预算。baseline-checks/result.json 必须绑定同一包指纹和 compat base。该预热构建不会上传；正式产物随后从新 ON commit 的干净 detached worktree 构建，不能沿用历史 dirty bundle。
...
node "$PKG/tools/compare.cjs" --run "$WORK/live/run" --out "$WORK/loader-comparison.json"
```
...
比较入口再次从本轮原始 run 运行 verifier，不接受人工改写的摘要作为新实测输入。它只读取本包内与原Git blob一致的CPU II裁决原件，不重新裁决历史实测、不合并两轮时间线、不改写旧结果。归档器会再算 LOADER-COMPARISON.json，发布门禁也会从归档 run 重算并核对，不因文件内写有成功标签就放行。

比较必须区分：首个准入样本、首个实际readerLoad调用、首次调用之后的调用。CPU II首个准入点没有加载reader，首个实际加载在第二点，不能把这两者当成同一件事。只有calls=1的阶段区间才进入对应构建调用的统计；call=0的非零区间单独保留为边界检查工作。后继尾部最多3个，4th尾部不补齐。新报告的旧投影差异会单列，不因为CPU诊断可用就声称库存对拍全过。

后续加载中位数的高/低只作描述，要求两轮都至少有两次首调之后的实际调用；不足时写 insufficient_following_invocations，不追加点、不更换窗口。不同运行的库存、任务、引擎状态和测量噪声并未控制，不输出精确提速百分比，不从8→2次factory执行推算引擎收益。首调指非fault报告中的首次实际调用，不证明引擎冷启动。
...
位置：openspec/changes/empire-treasury-core-rewrite/evidence/legacy-read-bridge-i/compat-cpu-recheck-iv。
...
Compat 若进入ON，则最终恰好两个config提交且OFF tree等于起始ae15ec5 tree；未进入ON则可不前移。Refactor仅新增本轮evidence的一个提交。提交tree必须等于门禁通过时index tree。
...
允许：CPU_DIAGNOSTIC_CAPTURE_VERIFIED / LOADER_IV_COMPARISON_RECORDED / RESTORED_BYTES_AND_RUNTIME_VERIFIED。
LOADER_IV_COMPARISON_RECORDED只说明比较按事实生成，不代表优化必然有效；ENGINE_CPU_BUDGET_GAP_UNRESOLVED仍保留，完整兼容验收不在本轮范围内。
