...
function fixture(opts={}){
 ...
 const ports={tick:()=>time,shard:()=>opts.shard||'shard1',resources:()=>['energy','H','O','X'],
  ...
  readers:()=>{calls.readerLoad++;charge(cost.readerLoad);if(opts.loadThrow)throw new Error('loader');return {
   buildObservation:({rooms:actual,compatDiagnostics:d})=>{calls.observationBuild++;charge(cost.observationBuild);if(opts.obsThrow)throw new Error('observation');
    d?.boundary('observationSetup');d?.boundary('observationRooms');d?.work({observationRooms:actual.length,observationLocations:actual.length*2,observationExistingLocations:actual.length*2,observationResourceKeys:actual.length*4});d?.boundary('observationFinalize');d?.boundary('observationView');d?.boundary();
   ...
   buildCommitments:({tasks,compatDiagnostics:d})=>{calls.commitmentBuild++;charge(cost.commitmentBuild);if(opts.commitThrow)throw new Error('commitment');const value=Object.keys(memory.data.resourceControl.tasks).length;
    d?.boundary('commitmentSetup');d?.boundary('commitmentTasks');d?.work({commitmentTaskRecords:Object.keys(tasks||{}).length,commitmentPendingTaskRecords:Object.keys(tasks||{}).length,commitmentTaskInvalidEvents:0,commitmentDemandHealthChecks:Object.keys(tasks||{}).length,commitmentReceiverHealthChecks:Object.keys(tasks||{}).length,commitmentRouteCandidates:Object.keys(tasks||{}).length});d?.boundary('commitmentReservations');d?.work({commitmentReservationRecords:0,commitmentReservationInvalidEvents:0,commitmentOwnerEvaluations:0,commitmentActiveReservations:0});d?.boundary('commitmentFinalize');d?.work({commitmentScopeBuckets:value?2:0,commitmentRoomBuckets:value?2:0,commitmentMergeRoutes:value?1:0,commitmentReservationScopes:0});d?.boundary();
