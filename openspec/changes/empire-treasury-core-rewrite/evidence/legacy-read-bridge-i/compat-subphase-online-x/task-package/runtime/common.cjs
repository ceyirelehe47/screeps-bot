...
const CANDIDATE_BASE = '57987cda22553fcdc842e8d4c72a98b6e94e00fb';
...
function loadSecret(p) {
  const st = fs.lstatSync(p); if (!st.isFile() || st.isSymbolicLink()) fail('CREDENTIAL_FILE_INVALID');
  ...
  return { token:s.token, server:EXPECTED.server, path:path.resolve(p), redact:redactor([s.token]) };
}
...
function assertCandidate(repo, sourceHead = CANDIDATE_BASE) {
  if (process.env.DEST || process.env.DEPLOY_ALLOW_DIRTY) fail('UNSAFE_BUILD_ENVIRONMENT');
  if (!/^[0-9a-f]{40}$/.test(sourceHead)) fail('SOURCE_HEAD_INVALID');
  ...
  git(repo,['merge-base','--is-ancestor',CANDIDATE_BASE,sourceHead]);
  git(repo,['merge-base','--is-ancestor',sourceHead,head]);
  const changed = git(repo,['diff','--name-only',sourceHead,head]).split('\n').filter(Boolean);
  if (changed.some(p => p !== 'src/runtime/treasuryCompatConfig.ts')) fail('CANDIDATE_SCOPE_CHANGED');
  return {head, tree, guard, sourceHead};
}
...
Object.assign(module.exports,{code,bytes,durable,log,json:readJson,same:(a,b)=>JSON.stringify(a)===JSON.stringify(b)});
