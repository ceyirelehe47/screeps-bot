...
function temp(t) { const p = C.fs.mkdtempSync(C.path.join(os.tmpdir(), 'read-V-')); t.after(() => C.fs.rmSync(p, { recursive: true, force: true })); return p; }
function init(repo, branch) {
  C.fs.mkdirSync(repo); C.git(repo, ['init', '-q']); C.git(repo, ['checkout', '-qb', branch]);
  C.git(repo, ['config', 'user.name', 'Read V fixture']); C.git(repo, ['config', 'user.email', 'fixture@example.invalid']); C.git(repo, ['config', 'commit.gpgsign', 'false']);
...
function fakeValidation(f) {
  ...
  C.writeNew(C.path.join(tests, 'summary.json'), { status: 'READ_V_FIXED_TESTS_VERIFIED', packageFingerprint: fingerprint });
  C.writeNew(C.path.join(checks, 'summary.json'), { status: 'READ_V_FULL_CHECKS_VERIFIED', packageFingerprint: fingerprint });
  C.writeNew(C.path.join(ab, 'characterization.json'), { status: 'READ_V_REAL_CORE_AB_VERIFIED', packageFingerprint: fingerprint, engineCpuGapRepaired: false, operationCounts: { baselineDirectStoreProperties: 24, candidateDirectStoreProperties: 4, coreStorePropertiesEach: 12, apiCallSequenceUnchanged: true }, scenarios: Array.from({ length: 20 }, (_, i) => ({ name: 'synthetic-workflow-' + i, byteEquivalent: true, writes: [0, 0], oldFactoryExecutions: 30, newFactoryExecutions: 7 })) });
...
test('workflow V package integrity uses exact case-sensitive paths without case-colliding fixtures', () => { assert.equal(C.integrity().status, 'PACKAGE_INTEGRITY_VERIFIED'); });
test('workflow V output packets retain trailing whitespace bytes losslessly', () => {
...
test('workflow V exact pinned files accept patch then reproduce all twelve payload files', t => {
  const f = fixture(t); const p = C.path.join(C.ROOT, 'patches/0001-read-context-and-endpoint-reuse.patch');
  C.git(f.compat, ['-c', 'core.autocrlf=false', 'apply', '--check', p]); C.git(f.compat, ['-c', 'core.autocrlf=false', 'apply', p]); assert.equal(f.w.checkSource(f.compat).files, 12);
});
test('workflow V real archive stage native-check commit push and readback end to end', t => {
  const f = fixture(t); assert.equal(f.w.baseline(f.compat, f.refactor).status, 'READ_V_BASELINES_VERIFIED'); assemble(f);
  ...
  assert.equal(f.w.postCommit(f.compat, f.refactor).status, 'READ_V_COMMITS_VERIFIED'); assert.equal(f.w.publish(f.compat, f.refactor).status, 'READ_V_PUBLISHED_NOT_DEPLOYED');
  assert.equal(f.w.publish(f.compat, f.refactor).status, 'READ_V_PUBLISHED_NOT_DEPLOYED'); // idempotent readback, not a second experiment
});
test('workflow V unknown dirty source input is refused before any snapshot or write', t => {
...
test('workflow V existing snapshot target is never deleted or overwritten', t => {
...
test('workflow V fixed write failure restores original CRLF bytes transactionally', t => {
...
test('workflow V additional untracked source file is refused at staging', t => {
...
test('workflow V changed payload file is rejected without staging it', t => {
...
test('workflow V a second source commit cannot be silently adopted', t => {
...
test('workflow V failed full-check artifact cannot pass the archive gate', t => {
...
test('workflow V raw packet tampering is detected before archiving', t => {
...
test('workflow V archive must retain the exact implementation package bytes', t => {
...
test('workflow V native whitespace gate rejects ordinary trailing whitespace with no exclusions', t => {
...
test('workflow V existing archive requires verify not destructive reassembly', t => {
...
test('workflow V remote branch drift cannot trigger force push', t => {
...
test('workflow V package paths reject traversal and repository-contained outputs', t => {
...
test('workflow V system and global autocrlf true do not change payload or staged archive', t => {
...
test('workflow V final result cannot claim an engine CPU repair even with refreshed inventory', t => {
...
test('workflow V extra files cannot enter the archive by extending its inventory', t => {
...
test('workflow V archive rechecks the deterministic operation-count evidence at staging', t => {
  const f = fixture(t); sourceCommit(f); const v = fakeValidation(f), p = C.path.join(v.ab, 'characterization.json'), x = C.json(p);
  x.operationCounts.coreStorePropertiesEach = 0; C.fs.writeFileSync(p, C.encode(x));
  assert.throws(() => f.w.archive(f.compat, f.refactor, v.tests, v.checks, v.ab), /AB_STORE_COUNTS_INVALID/);
});
