...
function workflow(policy = C.json(C.path.join(C.ROOT, 'references/source-lock.json')), root = C.ROOT) {
  ...
  function baseline(compat, refactor) {
    ...
    return { status: 'READ_V_BASELINES_VERIFIED', compatBase: policy.compatBase, refactorBase: policy.refactorBase };
  }
  function checkSource(compat, staged = false) {
    ...
    return { status: 'READ_V_SOURCE_BYTES_VERIFIED', staged, files: files.length, budgetChanged: false, directReadChanged: true };
  }
  ...
  function accepted(testDir, checksDir, abDir) {
    ...
    C.check(t.status === 'READ_V_FIXED_TESTS_VERIFIED' && p.status === 'READ_V_FULL_CHECKS_VERIFIED' && ab.status === 'READ_V_REAL_CORE_AB_VERIFIED', 'REQUIRED_CHECK_MISSING');
    ...
    C.check(ab.scenarios.every(x => x.oldFactoryExecutions === 30 && x.newFactoryExecutions === 7), 'AB_FACTORY_COUNT_MISMATCH');
    C.check(ab.operationCounts?.baselineDirectStoreProperties === 24 && ab.operationCounts?.candidateDirectStoreProperties === 4 && ab.operationCounts?.coreStorePropertiesEach === 12 && ab.operationCounts?.apiCallSequenceUnchanged === true, 'AB_STORE_COUNTS_INVALID');
  ...
  function archive(compat, refactor, testDir, checksDir, abDir) {
    ...
    const final = { status: 'READ_V_OFFLINE_VERIFIED_NOT_DEPLOYED', compatHead: h, compatBase: policy.compatBase, refactorBase: policy.refactorBase,
      packageFingerprint: fingerprint(), factoryBodiesChanged: ['commitments: explicit context only'], runtimeDefinitionReuse: true,
      firstSuccessfulFactoryExecutions: 7, followingFactoryExecutions: 0, actualEngineMeasurement: false,
      directReadOptimized: true, engineCpuGapRepaired: false, budget: 2, enabled: false,
      originalCpuRecheckIV: { raw: 4, diagnostic: 4, complete: 0, commitmentBuilds: 2, recoveryClosed: true } };
    plan.set('FINAL-VERIFICATION.json', C.encode(final));
    plan.set('EXECUTION-REPORT.md', Buffer.from('# Compat Read Optimization V\n\n' +
  ...
  function verifyArchive(refactor, staged = false, committedHead = null) {
    ...
    C.check(final.status === 'READ_V_OFFLINE_VERIFIED_NOT_DEPLOYED' && final.enabled === false && final.budget === 2 &&
      final.engineCpuGapRepaired === false && final.actualEngineMeasurement === false && final.firstSuccessfulFactoryExecutions === 7 &&
      final.followingFactoryExecutions === 0 && final.packageFingerprint === fingerprint(), 'FINAL_SCOPE_CLAIM_INVALID');
    ...
    return { status: 'READ_V_ARCHIVE_VERIFIED', files: all.length, staged, committedHead, nativeWhitespaceCheck: revision === null ? 'not-run' : 'passed-zero-exceptions', packageFingerprint: fingerprint() };
  }
  ...
  function postCommit(compat, refactor) { const ch = committed(compat); C.clean(refactor); C.check(C.gt(refactor, ['branch', '--show-current']) === policy.refactorBranch, 'WRONG_REFACTOR_BRANCH'); const rh = C.gt(refactor, ['rev-parse', 'HEAD']);
    ...
    const v = C.json(C.path.join(refactor, policy.evidenceTarget, 'FINAL-VERIFICATION.json')); C.check(v.compatHead === ch && v.packageFingerprint === fingerprint(), 'ARCHIVE_SOURCE_IDENTITY_MISMATCH'); return { status: 'READ_V_COMMITS_VERIFIED', compatHead: ch, refactorHead: rh };
  }
  function publish(compat, refactor) { const v = postCommit(compat, refactor);
    ...
    } return { ...v, status: 'READ_V_PUBLISHED_NOT_DEPLOYED' };
  }
