...
function tests(o) {
  ...
  const v = { status: 'READ_V_FIXED_TESTS_VERIFIED', readerTests: set.readerTests, workflowTests: set.workflowTests,
...
function characterize(o) {
  ...
  C.check(scenarios.every(x => x.oldFactoryExecutions === 30 && x.newFactoryExecutions === 7), 'AB_FACTORY_COUNT_MISMATCH');
  const beforeReads = support.measureStoreReads(support.oldText()), afterReads = support.measureStoreReads(support.newText());
  C.check(beforeReads.directStorePropertyReads === 24 && afterReads.directStorePropertyReads === 4 && beforeReads.coreStorePropertyReads === 12 && afterReads.coreStorePropertyReads === 12, 'STORE_PROPERTY_COUNT_MISMATCH');
  C.check(beforeReads.line === afterReads.line && JSON.stringify(beforeReads.methods) === JSON.stringify(afterReads.methods), 'STORE_METHOD_OR_OUTPUT_CHANGED');
  const operationCounts = { baselineDirectStoreProperties: 24, candidateDirectStoreProperties: 4, coreStorePropertiesEach: 12,
    apiCallSequenceUnchanged: true, scope: 'synthetic two-room two-resource single sample; not engine CPU' };
  const r = { status: 'READ_V_REAL_CORE_AB_VERIFIED', packageFingerprint: w.fingerprint(), scenarios,
    oldGeneratedBlob: C.blob(Buffer.from(support.oldText())), newGeneratedBlob: C.blob(Buffer.from(support.newText())),
    engineCpuGapRepaired: false, engineTimingMeasured: false, directReadOptimized: true, operationCounts };
  C.writeNew(C.path.join(out, 'characterization.json'), r); return { status: r.status, scenarios: scenarios.length, firstFactories: 7, subsequentFactories: 0 };
}
function fullCheck(o) {
  ...
  const v = { status: 'READ_V_FULL_CHECKS_VERIFIED', packageFingerprint: w.fingerprint(), compatBase: w.policy.compatBase,
