...
function baselineOffline(repo,out){
 ...
 U.clean(repo);
 const result={status:'COMPAT_BASELINE_OFFLINE_VERIFIED',head:K.COMPAT,repositoryNodeTests:200,loaderRegenerated:true,tests:{suites:195,tests:685},packageFingerprint:C.sha256(fs.readFileSync(path.join(U.ROOT,'INTEGRITY.json'))),atMs:Date.now()};C.writeNew(path.join(out,'result.json'),result);return result;
}
...
function runRepositoryTests(repo,out){
 if(process.env.DEST||process.env.DEPLOY_ALLOW_DIRTY||process.env.NODE_OPTIONS||process.env.NODE_PATH||process.env.CPU_DIAG_FIXTURE_ROOT)C.fail('UNSAFE_BUILD_OR_NODE_ENVIRONMENT');
 const files=['bridge.spec.cjs','real-readers.spec.cjs','independent.spec.cjs','loader-optimization.spec.cjs','build-optimization.spec.cjs'].map(x=>path.join(repo,'test/treasury-compat',x));
 ...
 if(r.status!==0||r.error||count('tests')!==200||count('pass')!==200||['fail','skipped','todo','cancelled'].some(n=>count(n)!==0))C.fail('REPOSITORY_NODE_TEST_GATE_FAILED');
 return {tests:200,passed:200};
}
