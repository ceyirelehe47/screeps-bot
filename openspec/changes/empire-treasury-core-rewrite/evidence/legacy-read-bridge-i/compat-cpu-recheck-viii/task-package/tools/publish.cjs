...
function verifyArchive(repo,{staged=false}={}){
 ...
 if(m.kind!=='cpu-recheck-VIII-archive/v1'||m.refactorBase!==K.REFACTOR||m.compatBase!==K.COMPAT||m.packageFingerprint!==C.sha256(fs.readFileSync(path.join(U.ROOT,'INTEGRITY.json'))))C.fail('ARCHIVE_IDENTITY_CHANGED');
 ...
 if(!C.same(C.readJson(path.join(dir,'BUILD-COMPARISON.json')),recomputed))C.fail('COMPARISON_EVIDENCE_CHANGED');
 return m;
}
...
function publish(refactor,compat,{out,push=false}={}){
 ...
 if(head===K.REFACTOR){gate=stageGate(refactor,compat);U.git(refactor,['commit','-m','evidence(compat): CPU recheck VIII capture and recovery']);head=U.textGit(refactor,['rev-parse','HEAD']);
