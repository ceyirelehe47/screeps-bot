...
async function prepare(o){
 ...
 const offline=C.readJson(path.join(o.offline,'result.json'));if(offline.head!==K.COMPAT||offline.status!=='COMPAT_BASELINE_OFFLINE_VERIFIED'||offline.repositoryNodeTests!==200||offline.packageFingerprint!==C.sha256(fs.readFileSync(path.join(U.ROOT,'INTEGRITY.json'))))C.fail('OFFLINE_BASELINE_GATE_MISSING');
 const selfTests=C.readJson(path.join(o.tests,'summary.json'));if(selfTests.status!=='CPU_DIAGNOSTIC_TOOL_TESTS_VERIFIED'||selfTests.tests!==require('../references/test-contract.json').count||selfTests.passed!==selfTests.tests||['failed','skipped','todo','cancelled'].some(k=>selfTests[k]!==0)||selfTests.packageFingerprint!==C.sha256(fs.readFileSync(path.join(U.ROOT,'INTEGRITY.json'))))C.fail('PACKAGE_TEST_GATE_MISSING');
 ...
 try{
  ...
  U.git(compat,['add','--',K.CONFIG]);U.git(compat,['commit','-m',`evidence(compat): bind CPU recheck VIII ${profile.startTick}-${profile.endTick}`]);U.clean(compat);
