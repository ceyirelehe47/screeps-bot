...
function typescript(){
 if(process.env.CPU_I_TS_REPO)return createRequire(path.resolve(process.env.CPU_I_TS_REPO,'package.json'))('typescript');
 if(process.env.CPU_I_TS_MODULE)return require(process.env.CPU_I_TS_MODULE);
...
function loadReader(variant='new',extras={}){
 const prefix=variant==='old'?'baseline':'implementation';
 const accounting=compile(fs.readFileSync(path.join(ROOT,'implementation/src/runtime/treasuryCompatCpu.ts'),'utf8'),'treasuryCompatCpu.ts').exports;
 const context=compile(fs.readFileSync(path.join(ROOT,prefix,'src/runtime/treasuryCompatRead.ts'),'utf8'),'treasuryCompatRead.ts',{
