...
async function prepare(o){
 ...
 const offline=C.readJson(path.join(o.offline,'result.json'));if(offline.sourceHead!==source.head||offline.status!=='ENVELOPE_XII_OFFLINE_VERIFIED'||offline.repositoryNodeTests!==290||offline.sourceManifestOutputValidation!=='all-listed-outputs'||offline.packageFingerprint!==C.sha256(fs.readFileSync(path.join(U.ROOT,'INTEGRITY.json'))))C.fail('OFFLINE_BASELINE_GATE_MISSING');
 const selfTests=C.readJson(path.join(o.tests,'summary.json'));if(selfTests.status!=='ENVELOPE_XII_PACKAGE_TESTS_VERIFIED'||selfTests.tests!==require('../references/test-contract.json').count||selfTests.passed!==selfTests.tests||['failed','skipped','todo','cancelled'].some(k=>selfTests[k]!==0)||selfTests.packageFingerprint!==C.sha256(fs.readFileSync(path.join(U.ROOT,'INTEGRITY.json'))))C.fail('PACKAGE_TEST_GATE_MISSING');
 ...
 try{
  ...
  U.git(compat,['add','--',K.CONFIG]);U.git(compat,['commit','-m',`evidence(compat): bind diagnostic envelope XII online II ${profile.startTick}-${profile.endTick}`]);U.clean(compat);
  ...
  const r={status:'ENVELOPE_XII_PREPARED_NOT_DEPLOYED',runId,profileHead:head,startTick:profile.startTick,endTick:profile.endTick,latestObservedTick:current.observedTick,work};
