...
function verifyArchive(repo,{staged=false}={}){
 ...
 if(m.kind!=='compat-diagnostic-envelope-optimization-XII-online-II-archive/v1'||m.refactorBase!==K.REFACTOR||m.compatBase!==K.COMPAT||!/^[0-9a-f]{40}$/.test(m.sourceHead||'')
  ||!/^[0-9a-f]{40}$/.test(m.compatClosedHead||'')||m.packageFingerprint!==C.sha256(fs.readFileSync(path.join(U.ROOT,'INTEGRITY.json')))
  ||m.implementationFingerprint!==C.sha256(fs.readFileSync(path.join(U.ROOT,'references/implementation-lock.json')))||m.readRetry?.readOnly!==true||m.readRetry?.writeRetries!==0)C.fail('ARCHIVE_IDENTITY_CHANGED');
 ...
 const M=require('./compare.cjs'),run=path.join(dir,'run');
 if(!C.same(C.readJson(path.join(dir,'SUBPHASE-ATTRIBUTION.json')),M.makeAttribution(run)))C.fail('ATTRIBUTION_EVIDENCE_CHANGED');
 if(!C.same(C.readJson(path.join(dir,'ENVELOPE-COMPARISON.json')),M.makeComparison(run)))C.fail('COMPARISON_EVIDENCE_CHANGED');
 return m;
}
function verifySourceCommit(compat,sourceHead){
 if(U.textGit(compat,['rev-parse',sourceHead+'^'])!==K.COMPAT)C.fail('SOURCE_COMMIT_PARENT_CHANGED');
 const names=U.textGit(compat,['diff','--name-only',K.COMPAT,sourceHead]).split('\n').filter(Boolean).sort();
 if(JSON.stringify(names)!==JSON.stringify([...X.IMPL.paths].sort()))C.fail('SOURCE_COMMIT_SCOPE_CHANGED');
 if(U.textGit(compat,['show','-s','--format=%s',sourceHead])!==X.IMPL.commitMessage)C.fail('SOURCE_COMMIT_MESSAGE_CHANGED');
 return U.textGit(compat,['rev-parse',sourceHead+'^{tree}']);
}
function sourceClosed(compat,m){
 ...
 const head=U.textGit(compat,['rev-parse','HEAD']),sourceTree=verifySourceCommit(compat,m.sourceHead);if(sourceTree!==m.sourceTree)C.fail('SOURCE_TREE_CHANGED');
 if(head===m.sourceHead){if(m.compatClosedHead!==m.sourceHead)C.fail('ARCHIVE_CLOSE_HEAD_MISMATCH');X.verifyHead(compat);return head;}
 if(head!==m.compatClosedHead||U.textGit(compat,['rev-parse','HEAD~2'])!==m.sourceHead||U.textGit(compat,['rev-parse','HEAD~3'])!==K.COMPAT)C.fail('COMPAT_CHAIN_CHANGED');
 const on=U.textGit(compat,['rev-parse','HEAD^']);if(U.textGit(compat,['rev-parse',on+'^'])!==m.sourceHead
  ||U.textGit(compat,['rev-parse','HEAD^{tree}'])!==sourceTree||U.textGit(compat,['diff','--name-only',m.sourceHead,on])!==K.CONFIG
  ||U.textGit(compat,['diff','--name-only',on,head])!==K.CONFIG)C.fail('COMPAT_NOT_EXACT_OFF_CLOSE');
 return head;
}
function stageGate(refactor,compat){
 ...
 return {status:'ENVELOPE_XII_STAGED_BYTES_AND_WHITESPACE_VERIFIED',tree:U.textGit(refactor,['write-tree']),files:staged.length,whitespaceExceptions:0};
}
function publish(refactor,compat,{out,push=false}={}){
 ...
 if(head===K.REFACTOR){gate=stageGate(refactor,compat);U.git(refactor,['commit','-m','evidence(compat): diagnostic envelope XII online II comparison and recovery']);head=U.textGit(refactor,['rev-parse','HEAD']);
 ...
 const result={status:push?'ENVELOPE_XII_EVIDENCE_PUSHED':'ENVELOPE_XII_EVIDENCE_COMMITTED',refactorHead:head,compatHead,sourceHead:m.sourceHead,gate,pushed:push};C.durable(out,result);return result;
}
module.exports={verifyArchive,stageGate,publish,sourceClosed,verifySourceCommit};
