...
const C=require('./common.cjs'),K=require('./policy.cjs'),S=require('./store.cjs'),W=require('./samples.cjs'),D=require('./decoder.cjs'),T=require('./tail-profile.cjs');
...
function summarize(samples,p){
 const tails=[];for(let i=1;i<samples.length;i++)if(samples[i-1]?.cpuProfile&&samples[i]?.previousCpuProfile)tails.push(T.mergeCompletion(samples[i-1].cpuProfile,samples[i].previousCpuProfile));
 const rows=samples.map(r=>({tick:r.tick,sampleOrdinal:r.cpuProfile?.sampleOrdinal,status:r.status,
  ...
  calls:r.cpuProfile?.calls||null,phaseIntervals:r.cpuProfile?.phases||null,attribution:r.cpuProfile?.attribution||null,
 ...
 const subphaseSummary=W.SUBPHASES.map(subphase=>{const values=rows.flatMap(x=>Object.hasOwn(x.attribution?.intervals||{},subphase)?[x.attribution.intervals[subphase]]:[]);
  return {subphase,invokedIntervals:values,median:median(values),count:values.length};});
 const workRows=rows.map(x=>({tick:x.tick,boundaries:x.attribution?.boundaries??0,active:x.attribution?.active??null,work:x.attribution?.work??{}}));
 return {rows,phaseSummary,subphaseSummary,workRows,attributionContract:{version:1,expectedCompleteBoundaries:12,subphases:W.SUBPHASES,workKeys:W.WORK_KEYS,perRecordCpuSampling:false,overlapsParentPhases:true},measurementBoundary:'inclusive intervals; accounting overhead retained; final snapshot-copy/return not measured',
...
function verifyRun(run){
 ...
 if(s.kind!==K.PUBLIC_KIND||s.refactorBase!==K.REFACTOR||s.compatRootBase!==K.COMPAT||s.compatBase!==s.sourceHead||!/^[a-f0-9]{40}$/.test(s.sourceHead)||!/^[a-f0-9]{40}$/.test(s.sourceTree)||s.wallLimitMs!==K.WALL_MS
 ...
 need(off?.status==='SOURCE_DEFAULT_OFF_RESTORED'&&off.sourceHead===s.sourceHead&&off.profileHead===s.profileHead&&off.sameTreeAsSource===true,'SOURCE_DEFAULT_OFF_UNCONFIRMED');
 ...
 return {status:captureVerified?'CPU_DIAGNOSTIC_CAPTURE_VERIFIED':'CPU_DIAGNOSTIC_CAPTURE_INCONCLUSIVE',captureVerified,
  closure:restoration.status,restoration,runId:s.runId,sourceHead:s.sourceHead,profileHead:s.profileHead,rawReports:raw,diagnosticReports:samples.length,
