...
function parseBridge(text){
 ...
 let value,encoding='raw-json',jsonText=text;
 try{value=JSON.parse(text);}catch{
  if(!text.includes('treasury-legacy-read-bridge'))return {kind:'ignored'};
  try{jsonText=decodeHtmlOnce(text);value=JSON.parse(jsonText);encoding='html-entities-single-layer';}
  catch(e){return {kind:'invalid',error:e instanceof C.Failure?e.code:'BRIDGE_JSON_INVALID'};}
 }
 return value?.kind==='treasury-legacy-read-bridge'?{kind:'bridge',encoding,jsonText,report:value}:{kind:'ignored'};
}
