...
async function collect({s,run,secret,api,WebSocketClass=global.WebSocket,maxLifeMs=3300000,readPolicy,readPause=C.pause,readNow}){
 ...
 function finish(reason,code){
  ...
  const result={kind:'cpu-diagnostic-collector-result/v1',runId:s.runId,pid:process.pid,status:code===0?'closed':'failed',reason,exitCode:code,closedAtMs:Date.now(),footerWritten,lockRemoved};
 ...
 try{
  ...
  ws.addEventListener('message',ev=>{if(closed)return;try{
