'use strict';
const C=require('./common.cjs'),K=require('./policy.cjs'),D=require('./decoder.cjs'),T=require('./tail-profile.cjs');
...
const SUBPHASES=Object.freeze(['observationSetup','observationRooms','observationFinalize','observationView','commitmentSetup','commitmentTasks','commitmentReservations','commitmentFinalize','projectionRows']);
const WORK_KEYS=Object.freeze(['observationRooms','observationLocations','observationExistingLocations','observationResourceKeys','commitmentTaskRecords','commitmentPendingTaskRecords','commitmentTaskInvalidEvents','commitmentDemandHealthChecks','commitmentReceiverHealthChecks','commitmentRouteCandidates','commitmentReservationRecords','commitmentReservationInvalidEvents','commitmentOwnerEvaluations','commitmentActiveReservations','commitmentScopeBuckets','commitmentRoomBuckets','commitmentMergeRoutes','commitmentReservationScopes','projectionRowsPlanned','projectionRowsCompleted','projectionIndexQueries']);
const equal=(a,b)=>Math.abs(a-b)<=1e-7*Math.max(1,Math.abs(a),Math.abs(b));

function attributionError(a,calls){
 if(!a)return calls.observationBuild===0&&calls.commitmentBuild===0?null:'CPU_ATTRIBUTION_MISSING';
 if(a.version!==1||!nn(a.boundaries)||a.boundaries>12||a.active!==null||!C.obj(a.intervals)||!C.obj(a.work))return 'CPU_ATTRIBUTION_IDENTITY_INVALID';
 if(Object.keys(a.intervals).some(k=>!SUBPHASES.includes(k)||!finite(a.intervals[k]))||Object.keys(a.work).some(k=>!WORK_KEYS.includes(k)||!nn(a.work[k])))return 'CPU_ATTRIBUTION_VALUE_INVALID';
 const has=k=>Object.hasOwn(a.intervals,k);
 if(calls.observationBuild===1&&(!['observationSetup','observationRooms','observationFinalize','observationView'].every(has)||a.boundaries<5))return 'CPU_OBSERVATION_ATTRIBUTION_INCOMPLETE';
 if(calls.observationBuild===0&&SUBPHASES.slice(0,4).some(has))return 'CPU_OBSERVATION_ATTRIBUTION_WITHOUT_CALL';
 if(calls.commitmentBuild===1&&(!SUBPHASES.slice(4).every(has)||a.boundaries!==12))return 'CPU_COMMITMENT_ATTRIBUTION_INCOMPLETE';
 if(calls.commitmentBuild===0&&SUBPHASES.slice(4).some(has))return 'CPU_COMMITMENT_ATTRIBUTION_WITHOUT_CALL';
 if((a.work.projectionRowsCompleted??0)>(a.work.projectionRowsPlanned??0))return 'CPU_PROJECTION_WORK_INVALID';
 return null;
}
function profileError(p,tick,ordinal,boundary){
 ...
 if(Object.keys(p.phases).some(k=>!PHASES.includes(k)||!finite(p.phases[k])))return 'CPU_PHASE_VALUE_INVALID';
 if(boundary==='afterRetention'&&p.completion==='tail_only'){const e=T.validateTailShape(p,tick,ordinal);if(e)return e;}
 else {if(p.completion!==undefined||!Object.hasOwn(p.phases,'legacyInputs')||!equal(Object.values(p.phases).reduce((n,x)=>n+x,0),p.elapsed))return 'CPU_PHASE_SUM_INVALID';
  const ae=attributionError(p.attribution,p.calls);if(ae)return ae;}
 ...
 if(p.elapsed>K.MAX_DIAGNOSTIC_OBSERVED_COST)return 'CPU_OBSERVED_SAFETY_STOP';return null;
}
function diagnosticError(r,p,i,previous=null,previousText=null){
 ...
 if(i===0){if(r.previousRun!==null||r.previousCpuProfile!==null)return 'FIRST_DIAGNOSTIC_NOT_FRESH';}
  ...
  try{T.mergeCompletion(b,a);}catch(err){return err.code||'CPU_TAIL_NOT_AN_EXTENSION';}
...
function sampleError(r,p){
 ...
 const a=r.cpuProfile?.attribution;if(a?.boundaries!==12||a.active!==null||a.work?.projectionRowsPlanned!==4||a.work?.projectionRowsCompleted!==4||a.work?.projectionIndexQueries!==16)return 'ATTRIBUTION_SAMPLE_INCOMPLETE';
...
module.exports={PHASES,CALLS,SUBPHASES,WORK_KEYS,attributionError,profileError,diagnosticError,sampleError,createWatch,health,equal};
