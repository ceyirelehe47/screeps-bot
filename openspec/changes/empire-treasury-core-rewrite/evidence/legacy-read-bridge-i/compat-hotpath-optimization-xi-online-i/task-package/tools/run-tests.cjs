...
function run(out,repo){
 ...
 const result={status:ok?'HOTPATH_XI_PACKAGE_TESTS_VERIFIED':'HOTPATH_XI_PACKAGE_TESTS_FAILED',tests:number('tests'),passed:number('pass'),failed:number('fail'),skipped:number('skipped'),todo:number('todo'),cancelled:number('cancelled'),
 ...
 C.writeNew(path.join(out,'summary.json'),result);if(!ok)C.fail('HOTPATH_XI_PACKAGE_TESTS_FAILED');return result;
}
