...
function verifyArchive(repo,{staged=false}={}){
 ...
 if(m.kind!=='compat-hotpath-optimization-XI-online-I-archive/v1'||m.refactorBase!==K.REFACTOR||m.compatBase!==K.COMPAT||!/^[0-9a-f]{40}$/.test(m.sourceHead||'')
 ...
 if(!C.same(C.readJson(path.join(dir,'HOTPATH-COMPARISON.json')),M.makeComparison(run)))C.fail('COMPARISON_EVIDENCE_CHANGED');
 return m;
}
...
function stageGate(refactor,compat){
 ...
 return {status:'HOTPATH_XI_STAGED_BYTES_AND_WHITESPACE_VERIFIED',tree:U.textGit(refactor,['write-tree']),files:staged.length,whitespaceExceptions:0};
}
function publish(refactor,compat,{out,push=false}={}){
 ...
 if(head===K.REFACTOR){gate=stageGate(refactor,compat);U.git(refactor,['commit','-m','evidence(compat): hotpath XI online I comparison and recovery']);head=U.textGit(refactor,['rev-parse','HEAD']);
 ...
 const result={status:push?'HOTPATH_XI_EVIDENCE_PUSHED':'HOTPATH_XI_EVIDENCE_COMMITTED',refactorHead:head,compatHead,sourceHead:m.sourceHead,gate,pushed:push};C.durable(out,result);return result;
}
