...
function loadRun(run,guard){
 ...
 if(s.kind!==K.KIND||!/^[a-f0-9]{32}$/.test(s.runId)||s.wallLimitMs!==K.WALL_MS
  ||s.refactorBase!==K.REFACTOR||s.compatRootBase!==K.COMPAT||!/^[a-f0-9]{40}$/.test(s.compatBase)||s.compatBase!==s.sourceHead||!/^[a-f0-9]{40}$/.test(s.sourceTree)||!/^[a-f0-9]{40}$/.test(s.profileHead)||!/^[a-f0-9]{40}$/.test(s.profileTree))C.fail('FORMAL_SESSION_INVALID');
