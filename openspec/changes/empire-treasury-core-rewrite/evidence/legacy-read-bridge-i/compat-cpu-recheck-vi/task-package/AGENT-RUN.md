# Compat CPU Recheck VI · Read V Engine Recheck · Agent 任务书
## 完整实现交付；复测已验收加载优化；四点、预算2、唯一候选上传与受控恢复

本轮不是再次实现 Read V，也不是完整兼容观察0004。唯一实现差异是使用已提交的Read V（私有样本上下文＋单次端点Store引用复用）；采样范围、检查点、预算和安全阈值与已成功收尾的CPU Recheck IV保持一致。比较只用于判断后续优化方向，不要求性能必须改善才能正确保存诊断。Read V作者变换器、生成核心、preview、测试和预算全部冻结，本轮不重应用V补丁。
...
以下动作限制针对工具新增动作与兼容桥写入，不要求停掉旧生产主循环的正常业务，也不授权修改旧业务以迎合测量。
...
- Read V generated blob: fe94ebece8f5118b76701007ce3906adfbb4da08
- Prior diagnostic evidence: 838ae7b170e8eedd4bd7a9f53e7ceee24e560ce1 的固定 diagnostic-verification.json，blob 6837368a22db1fe4de9c74be3120d6af106f59cb；只读历史对照，不复用旧窗口
- Prerequisites: references/prerequisites.json，基线工具会从真实Git对象库核验CPU IV与Read V原验收文件
...
两仓起始必须干净、本地与远端均匹配。不得用旧CPU IV工具直接运行新的源码，因为旧工具固定的生成核心和源码基线已失效；只使用本包。CPU I、Loader III 与 Read V 不再重应用。缺少固定对象/依赖或发现远端漂移时报告，不能自行换 base。
...
```bash
PKG="D:/code/screeps/incoming/screeps-compat-cpu-recheck-VI-2026-09-13"
REFACTOR="D:/code/screeps/screeps-bot"
COMPAT="D:/code/screeps/screeps-bot-compat-read-i"
WORK="D:/code/screeps/compat-cpu-recheck-VI-execution"
...
四条命令必须各自退出0。离线工具入口最多等待15分钟；这不是修改线上45分钟、75秒、HTTP时限或CPU门禁。固定工具测试为152项（128项已有用例适配当前身份和V核心 + 24项新增复测用例），以 references/test-contract.json 的文件集合与数量双重绑定；failed/skipped/todo/cancelled 均为0；摘要必须是 CPU_DIAGNOSTIC_TOOL_TESTS_VERIFIED。测试里有真实子进程、loopback假远端、真实读取器源代码与合成读端口，不属于线上CPU证据。

build 顺序：先独立执行仓库四份 Node specs（142/142，failed/skipped/todo/cancelled 均为0），然后检查已提交生成器的 --check 输出，随后 tsconfig.build.json、tsconfig.json 的 noEmit；原版 verify-jest-budget；Rollup build-only。要求195 suites / 685 tests，不能改预算。baseline-checks/result.json 必须绑定同一包指纹和 compat base。该预热构建不会上传；正式产物随后从新 ON commit 的干净 detached worktree 构建，不能沿用历史 dirty bundle。
...
node "$PKG/tools/compare.cjs" --run "$WORK/live/run" --out "$WORK/read-comparison.json"
```
...
报告应区分 rawReports / diagnosticReports / completeSamples；4点完整诊断最多得到3个由后继报告携带的afterRetention尾部。末点尾部必须保持unobservable，不能补第5点。首次sampleOrdinal=1仅证明模块生命周期首次准入，不是引擎冷启动证明。阶段是含测量开销的区间，call=0不能解释为函数已经以零CPU完成。READ-COMPARISON另列每点commitment实际调用、索引完整性、投影行数（期望两房×两资源=4）、完整样本判定；索引complete=true但rows=[]仍不是零承诺或完整样本。

即便4点都不完整，本轮仍可能成功定位阶段；不能据此写成兼容验收通过。若始终未进入commitment，其实际构建成本仍未知。不要由Node wall clock推导Screeps CPU，也不要把UTF-8遍历次数减半解释为整体CPU减半。

比较入口再次从本轮原始 run 运行 verifier，不接受人工改写的摘要作为新实测输入。它只读取本包内与原Git blob一致的CPU IV裁决原件，不重新裁决历史实测、不合并两轮时间线、不改写旧结果。归档器会再算 READ-COMPARISON.json，发布门禁也会从归档 run 重算并核对，不因文件内写有成功标签就放行。

比较必须区分：首个准入样本、首个实际readerLoad调用、首次调用之后的调用。本次直接历史对照是CPU IV：四点都加载reader，四点执行observation，两点执行commitment，但完整样本为零。更早CPU II的首个准入点没有加载reader，其原件只留作既有回归用例，不能作为VI的对照或借用样本。新运行也必须独立计算首次准入与首次实际加载，不能假设一定重合。只有calls=1的阶段区间才进入对应构建调用的统计；call=0的非零区间单独保留为边界检查工作。后继尾部最多3个，4th尾部不补齐。新报告的旧投影差异会单列，不因为CPU诊断可用就声称库存对拍全过。

后续加载中位数的高/低只作描述，要求两轮都至少有两次首调之后的实际调用；不足时写 insufficient_following_invocations，不追加点、不更换窗口。不同运行的库存、任务、引擎状态和测量噪声并未控制，不输出精确提速百分比，不从30→7次factory总数或24→4次Store属性访问推算引擎收益。首调指非fault报告中的首次实际调用，不证明引擎冷启动。

历史裁决未包含逐点commitment投影行数，因此新比较不反推历史行数，也不把II/IV原始网络日志重新拼装为本轮输入。

若compare独立失败而采集/恢复已通过，保留原采集和恢复事实，停止成功提交路径并回报；不得重新上线补实验。若它正常返回比较不确定（exit 2），按失败结果归档。失败结果允许如实归档，但不得使用成功标签。恢复未确认时优先保留/报告安全状态，不要继续另一次实验。
...
位置：openspec/changes/empire-treasury-core-rewrite/evidence/legacy-read-bridge-i/compat-cpu-recheck-vi。
...
Compat 若进入ON，则最终恰好两个config提交且OFF tree等于起始5080fda tree；未进入ON则可不前移。Refactor仅新增本轮evidence的一个提交。提交tree必须等于门禁通过时index tree。

若只在commit/push阶段失败，不重跑实验、不amend。原件与索引保持不动；确认失败点后，本工具可用新out路径复核已有唯一evidence提交并普通推送未到位分支，拒绝额外提交、远端第三方前移或force-push。

### 11. 回报内容

回报两个完整HEAD、普通push回读、包指纹、固定工具152/152、仓库Node142/142及195/685、绑定tick与4个实际tick、各阶段区间和首/后续差别、raw/diagnostic/complete三种计数、候选/恢复请求边界、恢复前后摘要与独立运行确认、原生whitespace=0，以及限制/偏差。

允许：CPU_DIAGNOSTIC_CAPTURE_VERIFIED / READ_VI_COMPARISON_RECORDED / RESTORED_BYTES_AND_RUNTIME_VERIFIED。
READ_VI_COMPARISON_RECORDED只说明比较按事实生成，不代表优化必然有效；ENGINE_CPU_BUDGET_GAP_UNRESOLVED仍保留，完整兼容验收不在本轮范围内。
