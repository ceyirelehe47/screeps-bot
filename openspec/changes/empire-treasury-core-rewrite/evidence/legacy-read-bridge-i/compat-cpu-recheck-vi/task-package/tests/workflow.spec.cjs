...
test('VI inherited integration: archived comparison is independently recomputed even after manifest hashes are updated',()=>{const f=make();try{archived(f);const dir=path.join(f.ref,K.EVIDENCE),name='READ-COMPARISON.json';const j=C.readJson(path.join(dir,name));j.currentCaptureVerified=true;const b=Buffer.from(JSON.stringify(j,null,2)+'\n');fs.writeFileSync(path.join(dir,name),b);const m=C.readJson(path.join(dir,'ARCHIVE-MANIFEST.json'));m.files[name]={bytes:b.length,sha256:C.sha256(b)};fs.writeFileSync(path.join(dir,'ARCHIVE-MANIFEST.json'),JSON.stringify(m,null,2)+'\n');assert.throws(()=>Pub.verifyArchive(f.ref),e=>e.code==='COMPARISON_EVIDENCE_CHANGED');}finally{f.cleanup();}});
test('VI inherited integration: archive includes and validates the added loader regeneration check packet',()=>{const f=make();try{assert.ok(A.CHECK_ALLOW.includes('loader-regenerate.exit.json'));archived(f);const packet=C.readJson(path.join(f.ref,K.EVIDENCE,'baseline-checks/loader-regenerate.stdout.json'));assert.equal(packet.sha256,C.sha256(packet.text));Pub.stageGate(f.ref,f.compat);}finally{f.cleanup();}});
