...
function runActual({old=false,instrument=false,enabled=true}={}){
 const file=old?'references/read-V/test/treasury-compat/fixtures/core-before-read-optimization-v.ts.txt':'references/treasuryCompatReadCore.generated.ts';
 const source=fs.readFileSync(path.join(root,file));assert.equal(C.blob(source),old?'7c20e7544bdc45bee11b1ee059ac5df2a12f2bf3':'fe94ebece8f5118b76701007ce3906adfbb4da08');
...
test('VI V-core integration: uninstrumented optimized real core emits four reports accepted by unchanged diagnostic validator',()=>{
...
test('VI V-core integration: real core four-point fixture executes 7 then 0 factories without cached observations',()=>{
 const r=runActual({instrument:true});assert.deepEqual(r.factoryCounts,[7,0,0,0]);assert.equal(r.calls,4);
...
test('VI V-core integration: previous real core fixture executes fourteen factories over four calls rather than seven',()=>{const r=runActual({old:true,instrument:true});assert.deepEqual(r.factoryCounts,[8,2,2,2]);assert.equal(r.executions.length,14);});
test('VI V-core integration: real reader plus optimized definitions retains original OFF admission behavior',()=>{const r=runActual({enabled:false,instrument:true});assert.equal(r.calls,0);assert.equal(r.executions.length,0);assert.equal(r.reports.length,0);assert.equal(r.f.calls.memory,0);});
test('VI V-core integration: optimized emitted profiles preserve three successor tails and no fourth tail',()=>{const r=runActual();assert.equal(r.reports[0].previousCpuProfile,null);for(let i=1;i<4;i++){assert.equal(r.reports[i].previousCpuProfile.tick,r.reports[i-1].tick);assert.equal(r.reports[i].previousCpuProfile.boundary,'afterRetention');}assert.equal(r.reports.filter(x=>x.previousCpuProfile).length,3);});
