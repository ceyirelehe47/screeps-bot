'use strict';
/** Offline interpretation of a freshly reverified run against the exact accepted CPU IV
...
function compareResults(current,prior){
 ...
 return {status:current.captureVerified?'READ_VI_COMPARISON_RECORDED':'READ_VI_COMPARISON_INCONCLUSIVE',
...
function makeComparison(run){
 ...
 result.currentReportDetails=[];
 result.historicalProjectionDetails='not included in the pinned IV verdict; original raw history is not reconstructed here';
 const selected=new Set();
 const publicSession=S_readPublic(run);
 ...
 for(const f of rawFrames){
  ...
  for(const raw of parsed[1]?.messages?.log||[]){const decoded=D.parseBridge(raw);if(decoded.kind!=='bridge'||!verified.receivedTicks?.includes(decoded.report.tick))continue;
   if(!selected.has(decoded.report.tick)){
    selected.add(decoded.report.tick);
    result.currentReportDetails.push(reportDetail(decoded.report,publicSession.profile));
   }
 ...
 result.currentProjectionSummary={
  reports:result.currentReportDetails.length,
  ...
  fullSamples:result.currentReportDetails.filter(x=>x.completeSample).length,
  countsAreDescriptionsNotAcceptanceGates:true
 };
...
module.exports={measures,compareResults,makeComparison,reportDetail};
