...
async function prepare(o){
 ...
 const offline=C.readJson(path.join(o.offline,'result.json'));if(offline.head!==K.COMPAT||offline.status!=='COMPAT_BASELINE_OFFLINE_VERIFIED'||offline.repositoryNodeTests!==142||offline.packageFingerprint!==C.sha256(fs.readFileSync(path.join(U.ROOT,'INTEGRITY.json'))))C.fail('OFFLINE_BASELINE_GATE_MISSING');
 ...
 try{
  ...
  U.git(compat,['add','--',K.CONFIG]);U.git(compat,['commit','-m',`evidence(compat): bind CPU recheck VI ${profile.startTick}-${profile.endTick}`]);U.clean(compat);
