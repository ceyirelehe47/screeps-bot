...
function temp(t) { const p = C.fs.mkdtempSync(C.path.join(os.tmpdir(), 'build-VII-')); t.after(() => C.fs.rmSync(p, { recursive: true, force: true })); return p; }
function init(repo, branch) {
  C.fs.mkdirSync(repo); C.git(repo, ['init', '-q']); C.git(repo, ['checkout', '-qb', branch]);
  C.git(repo, ['config', 'user.name', 'Build VII fixture']); C.git(repo, ['config', 'user.email', 'fixture@example.invalid']); C.git(repo, ['config', 'commit.gpgsign', 'false']);
...
function fakeValidation(f) {
  ...
  C.writeNew(C.path.join(tests, 'summary.json'), { status: 'BUILD_VII_FIXED_TESTS_VERIFIED', packageFingerprint: fingerprint });
  C.writeNew(C.path.join(checks, 'summary.json'), { status: 'BUILD_VII_FULL_CHECKS_VERIFIED', packageFingerprint: fingerprint });
  C.writeNew(C.path.join(ab, 'characterization.json'), { status: 'BUILD_VII_REAL_CORE_AB_VERIFIED', packageFingerprint: fingerprint, engineCpuGapRepaired: false, operationCounts: C.json(C.path.join(C.ROOT, 'references/operation-contract.json')), scenarios: Array.from({ length: 20 }, (_, i) => ({ name: 'synthetic-workflow-' + i, byteEquivalent: true, writes: [0, 0], oldFactoryExecutions: 7, newFactoryExecutions: 7 })) });
...
test('workflow VII package integrity uses exact case-sensitive paths without case-colliding fixtures', () => { assert.equal(C.integrity().status, 'PACKAGE_INTEGRITY_VERIFIED'); });
test('workflow VII output packets retain trailing whitespace bytes losslessly', () => {
...
test('workflow VII exact pinned files accept patch then reproduce all eleven payload files', t => {
  const f = fixture(t); const p = C.path.join(C.ROOT, 'patches/0001-full-build-bucket-fusion.patch');
...
test('workflow VII real archive stage native-check commit push and readback end to end', t => {
  const f = fixture(t); assert.equal(f.w.baseline(f.compat, f.refactor).status, 'BUILD_VII_BASELINES_VERIFIED'); assemble(f);
  ...
  assert.equal(f.w.postCommit(f.compat, f.refactor).status, 'BUILD_VII_COMMITS_VERIFIED'); assert.equal(f.w.publish(f.compat, f.refactor).status, 'BUILD_VII_PUBLISHED_NOT_DEPLOYED');
  assert.equal(f.w.publish(f.compat, f.refactor).status, 'BUILD_VII_PUBLISHED_NOT_DEPLOYED'); // idempotent readback, not a second experiment
});
test('workflow VII unknown dirty source input is refused before any snapshot or write', t => {
...
test('workflow VII existing snapshot target is never deleted or overwritten', t => {
...
test('workflow VII fixed write failure restores original CRLF bytes transactionally', t => {
...
test('workflow VII additional untracked source file is refused at staging', t => {
...
test('workflow VII changed payload file is rejected without staging it', t => {
...
test('workflow VII a second source commit cannot be silently adopted', t => {
...
test('workflow VII failed full-check artifact cannot pass the archive gate', t => {
...
test('workflow VII raw packet tampering is detected before archiving', t => {
...
test('workflow VII archive must retain the exact implementation package bytes', t => {
...
test('workflow VII native whitespace gate rejects ordinary trailing whitespace with no exclusions', t => {
...
test('workflow VII existing archive requires verify not destructive reassembly', t => {
...
test('workflow VII remote branch drift cannot trigger force push', t => {
...
test('workflow VII package paths reject traversal and repository-contained outputs', t => {
...
test('workflow VII system and global autocrlf true do not change payload or staged archive', t => {
...
test('workflow VII final result cannot claim an engine CPU repair even with refreshed inventory', t => {
...
test('workflow VII extra files cannot enter the archive by extending its inventory', t => {
...
test('workflow VII archive rechecks the deterministic operation-count evidence at staging', t => {
  ...
  assert.throws(() => f.w.archive(f.compat, f.refactor, v.tests, v.checks, v.ab), /BUILD_OPERATION_COUNT_MISMATCH/);
});
