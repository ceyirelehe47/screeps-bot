...
function tests(o) {
  ...
  const reader = C.capture(out, 'reader-tests', ['--test', '--test-reporter=tap', ...['bridge', 'real-readers', 'loader-optimization', 'build-optimization'].map(spec)], { cwd: fixture, env: C.environment(modules) });
  ...
  const v = { status: 'BUILD_VII_FIXED_TESTS_VERIFIED', readerTests: set.readerTests, workflowTests: set.workflowTests,
...
function characterize(o) {
  ...
  const support = require(C.path.join(C.path.resolve(o.compat), 'test/treasury-compat/build-test-support.cjs'));
  const scenarios = Object.keys(support.S.scenarios).map(n => support.compareScenario(n));
  C.check(scenarios.length === 20 && scenarios.every(x => x.byteEquivalent && x.writes.every(n => n === 0)), 'AB_COMPARISON_FAILED');
  C.check(scenarios.every(x => x.oldFactoryExecutions === 7 && x.newFactoryExecutions === 7), 'AB_FACTORY_COUNT_MISMATCH');
  const operations = [0, 37, 256].map(count => {
    const [a, b] = support.operationCounts(count, 32);
    C.check(JSON.stringify(a.semantics) === JSON.stringify(b.semantics), 'FULL_INDEX_OPERATION_FIXTURE_MISMATCH');
    return { tasks: count, resourcesPerEndpointIncludingEnergyH: 34,
      baseline: { observation: a.observation, commitment: a.commitment },
      candidate: { observation: b.observation, commitment: b.commitment }, fullApiEquivalent: true };
  });
  const ar = support.S.measureStoreReads(support.beforeText()), br = support.S.measureStoreReads(support.afterText());
  C.check(ar.line === br.line && JSON.stringify(ar.methods) === JSON.stringify(br.methods), 'STORE_METHOD_OR_OUTPUT_CHANGED');
  const operationCounts = { scenarios: operations, directStorePropertiesEach: ar.directStorePropertyReads,
    coreStorePropertiesEach: ar.coreStorePropertyReads, candidateDirect: br.directStorePropertyReads, candidateCore: br.coreStorePropertyReads,
    apiCallSequenceUnchanged: true, scope: 'synthetic full builds; Map/array counters only, NOT engine CPU' };
  C.check(JSON.stringify(operationCounts) === JSON.stringify(C.json(C.path.join(C.ROOT, 'references/operation-contract.json'))), 'BUILD_OPERATION_COUNT_MISMATCH');
  const r = { status: 'BUILD_VII_REAL_CORE_AB_VERIFIED', packageFingerprint: w.fingerprint(), scenarios,
    oldGeneratedBlob: C.blob(Buffer.from(support.beforeText())), newGeneratedBlob: C.blob(Buffer.from(support.afterText())),
    engineCpuGapRepaired: false, engineTimingMeasured: false, directReadChanged: false,
    taskBucketStorageChanged: true, observationIntermediatesReduced: true, operationCounts };
  C.writeNew(C.path.join(out, 'characterization.json'), r);
  return { status: r.status, scenarios: scenarios.length, operationScenarios: operations.length, engineCpuGapRepaired: false };
}
function fullCheck(o) {
  ...
  const r = C.capture(out, 'repository-tests', ['--test', '--test-reporter=tap', ...['bridge', 'real-readers', 'independent', 'loader-optimization', 'build-optimization'].map(n => C.path.join(repo, 'test/treasury-compat/' + n + '.spec.cjs'))], { cwd: repo }); C.tap(r.stdout.text, set.repositoryTests);
  ...
  const v = { status: 'BUILD_VII_FULL_CHECKS_VERIFIED', packageFingerprint: w.fingerprint(), compatBase: w.policy.compatBase,
