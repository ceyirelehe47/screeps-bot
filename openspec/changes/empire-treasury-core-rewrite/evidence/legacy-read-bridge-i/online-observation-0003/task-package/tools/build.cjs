...
function baselineOffline(repo,out){
 ...
 U.clean(repo);
 const result={status:'COMPAT_BASELINE_OFFLINE_VERIFIED',head:K.COMPAT,tests:{suites:195,tests:685},atMs:Date.now()};C.writeNew(path.join(out,'result.json'),result);return result;
}
