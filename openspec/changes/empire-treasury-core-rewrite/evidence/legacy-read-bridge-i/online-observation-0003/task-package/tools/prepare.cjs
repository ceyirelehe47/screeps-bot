...
async function prepare(o){
 ...
 const offline=C.readJson(path.join(o.offline,'result.json'));if(offline.head!==K.COMPAT||offline.status!=='COMPAT_BASELINE_OFFLINE_VERIFIED')C.fail('OFFLINE_BASELINE_GATE_MISSING');
 const selfTests=C.readJson(path.join(o.tests,'summary.json'));if(selfTests.status!=='FORMAL_0003_TESTS_VERIFIED'||selfTests.packageFingerprint!==C.sha256(fs.readFileSync(path.join(U.ROOT,'INTEGRITY.json'))))C.fail('PACKAGE_TEST_GATE_MISSING');
 ...
 try{
  ...
  U.git(compat,['add','--',K.CONFIG]);U.git(compat,['commit','--no-gpg-sign','-m',`evidence(compat): bind observation 0003 ${profile.startTick}-${profile.endTick}`]);U.clean(compat);
  ...
  const current=await A.current(api,guard);validateProfileLead(profile,current.observedTick);
  ...
  const s={kind:'formal-compat-observation-0003/v1',runId,...C.EXPECTED,refactorBase:K.REFACTOR,compatBase:K.COMPAT,observedTick:live.time,
   profile,profileHead:head,profileTree:b.tree,build,backupBuild:current.build,preparedAtMs:Date.now(),wallLimitMs:K.WALL_MS,
  ...
  const pub={...s,kind:'formal-compat-observation-0003-public/v1',backupDigest:backup.digest,candidateDigest:candidate.digest};
  S.newRecord(run,'public-session.json',pub);
  const r={status:'FORMAL_0003_PREPARED_NOT_DEPLOYED',runId,profileHead:head,startTick:profile.startTick,endTick:profile.endTick,latestObservedTick:current.observedTick,work};
