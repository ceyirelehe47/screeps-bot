...
function session(){const head='a'.repeat(40),tree='b'.repeat(40),profile=K.profileFor(1000);
 ...
 const sourceHead='d'.repeat(40),sourceTree='e'.repeat(40);
 return {kind:K.KIND,runId:'c'.repeat(32),...C.EXPECTED,refactorBase:K.REFACTOR,compatRootBase:K.COMPAT,compatBase:sourceHead,sourceHead,sourceTree,profile,observedTick:1000,profileHead:head,profileTree:tree,
