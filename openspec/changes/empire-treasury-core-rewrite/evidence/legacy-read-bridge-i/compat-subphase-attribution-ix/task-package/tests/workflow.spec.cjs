...
function temp(t) { const p = C.fs.mkdtempSync(C.path.join(os.tmpdir(), 'subphase-IX-')); t.after(() => C.fs.rmSync(p, { recursive: true, force: true })); return p; }
function init(repo, branch) {
  C.fs.mkdirSync(repo); C.git(repo, ['init', '-q']); C.git(repo, ['checkout', '-qb', branch]);
  C.git(repo, ['config', 'user.name', 'Subphase IX fixture']); C.git(repo, ['config', 'user.email', 'fixture@example.invalid']); C.git(repo, ['config', 'commit.gpgsign', 'false']);
...
function fakeValidation(f) {
  ...
  C.writeNew(C.path.join(tests, 'summary.json'), { status: 'SUBPHASE_IX_FIXED_TESTS_VERIFIED', packageFingerprint: fingerprint });
  C.writeNew(C.path.join(checks, 'summary.json'), { status: 'SUBPHASE_IX_FULL_CHECKS_VERIFIED', packageFingerprint: fingerprint });
  C.writeNew(C.path.join(ab, 'characterization.json'), { status: 'ATTRIBUTION_IX_REAL_CORE_VERIFIED', packageFingerprint: fingerprint, engineCpuGapRepaired: false, onlineRun: false, perRecordCpuSampling: false, contract: C.json(C.path.join(C.ROOT, 'references/attribution-contract.json')), scenarios: Array.from({ length: 20 }, (_, i) => ({ name: 'synthetic-workflow-' + i, byteEquivalent: true, writes: [0, 0] })), profiles: Array.from({ length: 4 }, (_, i) => ({ name: 'profile-' + i, boundaries: 12, active: null, completeProjection: true })) });
...
test('workflow IX package integrity uses exact case-sensitive paths without case-colliding fixtures', () => { assert.equal(C.integrity().status, 'PACKAGE_INTEGRITY_VERIFIED'); });
test('workflow IX output packets retain trailing whitespace bytes losslessly', () => {
...
test('workflow IX exact pinned files accept patch then reproduce all twenty payload files', t => {
  const f = fixture(t); const p = C.path.join(C.ROOT, 'patches/0001-subphase-attribution-ix.patch');
  C.git(f.compat, ['-c', 'core.autocrlf=false', 'apply', '--check', p]); C.git(f.compat, ['-c', 'core.autocrlf=false', 'apply', p]); assert.equal(f.w.checkSource(f.compat).files, 20);
});
test('workflow IX real archive stage native-check commit push and readback end to end', t => {
  const f = fixture(t); assert.equal(f.w.baseline(f.compat, f.refactor).status, 'SUBPHASE_IX_BASELINES_VERIFIED'); assemble(f);
  ...
  assert.equal(f.w.postCommit(f.compat, f.refactor).status, 'SUBPHASE_IX_COMMITS_VERIFIED'); assert.equal(f.w.publish(f.compat, f.refactor).status, 'SUBPHASE_IX_PUBLISHED_NOT_DEPLOYED');
  assert.equal(f.w.publish(f.compat, f.refactor).status, 'SUBPHASE_IX_PUBLISHED_NOT_DEPLOYED'); // idempotent readback, not a second experiment
});
test('workflow IX unknown dirty source input is refused before any snapshot or write', t => {
...
test('workflow IX existing snapshot target is never deleted or overwritten', t => {
...
test('workflow IX fixed write failure restores original CRLF bytes transactionally', t => {
...
test('workflow IX additional untracked source file is refused at staging', t => {
...
test('workflow IX changed payload file is rejected without staging it', t => {
...
test('workflow IX a second source commit cannot be silently adopted', t => {
...
test('workflow IX failed full-check artifact cannot pass the archive gate', t => {
...
test('workflow IX raw packet tampering is detected before archiving', t => {
...
test('workflow IX archive must retain the exact implementation package bytes', t => {
...
test('workflow IX native whitespace gate rejects ordinary trailing whitespace with no exclusions', t => {
...
test('workflow IX existing archive requires verify not destructive reassembly', t => {
...
test('workflow IX remote branch drift cannot trigger force push', t => {
...
test('workflow IX package paths reject traversal and repository-contained outputs', t => {
...
test('workflow IX system and global autocrlf true do not change payload or staged archive', t => {
...
test('workflow IX final result cannot claim an engine CPU repair or online execution even with refreshed inventory', t => {
  const f = fixture(t); assemble(f); const target = C.path.join(f.refactor, f.policy.evidenceTarget), p = C.path.join(target, 'FINAL-VERIFICATION.json');
  const x = C.json(p); x.engineCpuGapRepaired = true; C.fs.writeFileSync(p, C.encode(x)); const m = C.json(C.path.join(target, 'ARCHIVE-MANIFEST.json'));
  m.files['FINAL-VERIFICATION.json'] = { bytes: C.fs.statSync(p).size, sha256: C.sha(C.fs.readFileSync(p)) }; C.fs.writeFileSync(C.path.join(target, 'ARCHIVE-MANIFEST.json'), C.encode(m));
  assert.throws(() => f.w.verifyArchive(f.refactor), /FINAL_SCOPE_CLAIM_INVALID/);
});
test('workflow IX extra files cannot enter the archive by extending its inventory', t => {
  const f = fixture(t); assemble(f); const target = C.path.join(f.refactor, f.policy.evidenceTarget), n = 'unrequested.txt', p = C.path.join(target, n);
  C.fs.writeFileSync(p, 'unrequested\n'); const m = C.json(C.path.join(target, 'ARCHIVE-MANIFEST.json'));
  m.files[n] = { bytes: C.fs.statSync(p).size, sha256: C.sha(C.fs.readFileSync(p)) }; C.fs.writeFileSync(C.path.join(target, 'ARCHIVE-MANIFEST.json'), C.encode(m));
  assert.throws(() => f.w.verifyArchive(f.refactor), /ARCHIVE_WHITELIST_MISMATCH/);
});
test('workflow IX archive rechecks the deterministic operation-count evidence at staging', t => {
  const f = fixture(t); sourceCommit(f); const v = fakeValidation(f), p = C.path.join(v.ab, 'characterization.json'), x = C.json(p);
  x.profiles[0].boundaries = 11; C.fs.writeFileSync(p, C.encode(x));
  assert.throws(() => f.w.archive(f.compat, f.refactor, v.tests, v.checks, v.ab), /ATTRIBUTION_PROFILE_INVALID/);
});

test('workflow IX attribution evidence cannot claim per-record CPU sampling or online measurement', t => {
  const f = fixture(t); sourceCommit(f); const v = fakeValidation(f), p = C.path.join(v.ab, 'characterization.json'), x = C.json(p);
  x.perRecordCpuSampling = true; C.fs.writeFileSync(p, C.encode(x));
  assert.throws(() => f.w.archive(f.compat, f.refactor, v.tests, v.checks, v.ab), /ATTRIBUTION_SCOPE_INVALID/);
});
