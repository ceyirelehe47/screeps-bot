...
describe("V1 基线反例：作用域声明 B、实际提交许可 A（同参数）", () => {
  it("事件必须只归实际提交的许可 A——基线上 entered/effect 被错记到 B", () => {
    ...
    // 基线工具路径：identity 与 fn 实际提交的许可分离
    const outcome = journal.runWithInvocation(b, args, () => service.executeAuthorizedDispatch(a.dispatch));
...
describe("V2 基线反例：捕获器收 J2 marker、adapter 实际用 J1", () => {
  it("恢复入口必须在对账前拒绝错来源——基线上世界 1000 却误判 committed", () => {
    ...
    const outcome = j1.runWithInvocation(a, args, () => service.executeAuthorizedDispatch(a.dispatch));
    interceptor.restore();
