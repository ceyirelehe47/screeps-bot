...
### 10.6 候选冻结与证据边界（Core Candidate Seal I）

生产实现自冻结基线 869149d 起零源码 diff：Seal I 只补测试侧证据能力
（H18 全程轨迹、unknown 风险事实独立基线与字段级比较、敏感性检查），
不新增持久字段、第二权威或协议，不改变任何生命周期设计。两点边界写明：

1. **收尾/恢复窗口数（40/10）是测试限值，不是调度性质**。它们是针对
   固定 H18 fixture 的有限回归测试限值，由该 fixture 义务量与每 tick
   ≤8 逻辑份额的静态推算加调度余量导出。每 tick 8 份额是上限而不是
   最低服务保证——由最大吞吐量推不出"最少获得多少服务"，因此不能用
   该推算宣称所有工作负载的完成时间上界；实际完成窗口数由每次运行
   如实记录。
2. **证据只在测试侧，产物 hash 只作追溯**。轨迹与风险基线经
   TREASURY_SEAL_EVIDENCE_DIR 写入测试运行目录，不进游戏 Memory；
   风险比较的白名单字段（identity/worstCase 完整腿/调用边界三件套/
   结论/义务归属）是既有字段语义，不是新 schema。构建器嵌入时间与
   Git 身份，bundle hash 不具字节复现性——hash 用于产物追溯，逻辑
   冻结以 git diff 为准。部署许可不在本轮授予范围；真实 driver 的
   非原子窗口等运行模型仍由 §4 边界约束（不因此轮封板复验而扩大）。

### 10.7 证据工具的证据完整性边界（Evidence Remediation I 勘误）

Seal I 的证据工具有两处缺口（由独立审查发现、基线反例证实），本轮在
测试侧补修——不涉及任何生产生命周期设计，生产继续零 diff：

1. **提取不得抹平"缺失"与"null"**（V1）：`sealUnknownRiskOf` 此前把
   字段 undefined 统一转成 null，"原始记录上字段不存在"与"字段存在且
   值为 null"（后者是合法原值）在提取与序列化后不可区分。修复后提取
   输出用显式哨兵（首尾 NUL 控制字符的字符串，JSON 可表示且不与生产
   值全等）表达缺失；基线建立（`sealBuildUnknownRiskBaseline`）遇到
   缺记录或缺白名单字段明确拒绝——基线与当前恰好都缺同一风险事实不
   构成"完整基线"。diff/错误产物把哨兵渲染为可解释标记，不让缺失在
   JSON 里消失。
2. **完整性核验必须用实际快照重算，不信任派生标签**（V2）：
   `sealVerifyTraceCompleteness` 此前只在 `riskCheckedIds !== null` 时
   检查覆盖标签，不比较检查点 `unknownRisk` 实际快照、中间 `riskDiff`
   不参与判定——风险证据整项缺失、实际快照漂移而标签仍报一致、中间
   保存非空差异被正确终态掩盖三种情况均漏报。修复后每个标准检查点
   （observe/bounded/recovery 的重载前后与 final-close 的 close 前后）
   必须有非 null 实际风险快照、覆盖 expected ID 集合，并与独立基线逐
   字段重算；`riskDiff=null` 的语义是"重算后无差异"，不是缺少
   `unknownRisk` 的合法理由；终态标签与 post-close 实际快照矛盾必须
   报错。核验纯读：不修改轨迹、不自愈缺失证据。
3. **历史兼容**：修复前导出的轨迹 JSON 在新核验器下仍通过（旧提取器
   恒定输出全部白名单键，重算与旧标签一致）——新检查只拒绝被破坏或
   欠证据的轨迹；合成轨迹（检查点无实际风险数据）不再作为完整性正例，
   仅用于欠缺证据的负向测试。两处缺口属于证据工具自身，不构成生产
   风险已漂移的证据；V1/V2/L01–L04 定位见 test-migration-map §15。
