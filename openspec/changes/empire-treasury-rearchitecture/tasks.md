...
- [ ] 14.1 durable intent 权威（intents.ts schema v1）：版本化 store（version/entryCount/key="i:"+id/容量 64/entry 全形状/canonical postings/phase 枚举/安全整数与聚合溢出/global reset 首次 load 全量验证/heap health cache/损坏与未知版本 fail closed/显式 repair 并入 faultResolution）；phase 状态机（authorized/ready/executing/returned_non_ok/ok_pending_commit/committed/aborted/execution_unknown/quarantined/resolution_pending——ready 与 executing 绝不混同）；写入失败 callback 零调用 + 槽位释放 + 结构化拒绝
- [ ] 14.2 slot 统一计数：recoverySlots = quarantine + intent + 无 intent 的 active handle 恒等式；prepare admission O(1)；intent 接管/fault 转换/commit-abort 回收全路径守恒（指标 intentSlotsRemaining 证明）
- [ ] 14.3 global reset 恢复：beginTick 显式分支加载验证；未完成 intent 阻断新 writer；ready 确认未执行关闭；其余保守转 execution-unknown quarantine；恢复幂等；quarantine 写失败时 intent 保留 postings/占用/slot（emergency intent authority）；query 与 readiness 同时计入 quarantine 与 unresolved intents
- [ ] 14.4 资源授权（authorization.ts）：结构化授权请求（actionKind/resource/rooms/amount/owner/withhold|policyFingerprint/allowIncoming 默认 false/必扣 outgoing+reservations/容量需求）；授权计算（observation+overlay−outgoing−reservations−quarantine/intent 风险−withhold−其它授权占用 + completeness/store health/readiness/容量/安全整数）；opaque token（heap-only/冻结/WeakSet/单次使用/tick+generation 有界/绑定 epoch+commitmentRevision+projectionRevision+quarantineRevision+ownerKey+policyFingerprint+scope+digest）；授权即占 budget 防超卖；失效矩阵测试（task/reservation mutation、projection commit、quarantine revision、owner、policy、epoch、跨 tick、跨 service、重复消费、伪造）
- [ ] 14.5 action contract + adapter registry（actionContracts.ts）：contract 冻结结构（canonical args 确定性派生 postings/structureIds/authorizationDigest/epoch 绑定）；adapter 契约（validate/derivePostings/execute/reconcile）与注册边界（仅 actionContracts.ts 与测试）；测试 adapter（多 posting fixture + 副作用计数 + 可配置 reconciler）；executeTreasuryActionContract 唯一生产入口（授权消费→intent→adapter 恰好一次→commit/abort）；executePreparedAction 降级 test-only（架构测试守护生产禁用）
- [ ] 14.6 quarantine per-transaction 保守聚合：Σ max(0,−net) 流出 / Σ max(0,net) 容量（跨 transaction 不抵消、transaction 内先合并）；每步安全整数（溢出 unhealthy + readiness false）；unresolved intents 并入同一口径；快照封闭（entry/list/聚合/health 冻结副本，不泄漏内部 Map）；写入前重验 entry
- [ ] 14.7 staged atomic resolution：resolution store v2（version/entryCount/完整 shape/load 验证/未知版本与损坏 fail closed/容量满先于状态变化拒绝/v1 无损升级）；staged 流程（slot 预检→resolving tombstone→结算动作→finalize marker+tombstone）；各阶段故障注入 + global reset 幂等恢复（不重复结算/不丢占用/不错误解锁）；not-executed 先写 final tombstone 再释放
- [ ] 14.8 既有 receipt 刷新：refreshSettledReceiptForResolution（own key 存在且有效时更新 settled tick 至 resolution tick；entryCount/updatedAt/nextExpiryTick 同步重算；receiptRefreshes 计数）；统一 replay horizon（heap receipt→Memory receipt→committed tombstone 窗口，同一 retention 常量；receipt 过期 + tombstone 在窗口内时 prepare 仍拒）
...
- [ ] 14.10 owner identity 统一 + reservation store 完整健康：treasuryOwnerIdentityKey 复用 canonical token 算法（移除 NUL 比较键；聚合/排除/mutation/migration 同一 key）；validateReservationStoreHealth 完整 load validation + heap cache/revision；version=4 ≠ healthy；mutation 顺序（输入验证→schema gate→store 健康→写入）；只读 API 冻结快照 + 旧兼容读取不传播 NaN
- [ ] 14.11 capacity view 分立：strictProjectedUsedCapacity/strictProjectedFreeCapacity（used+free=physical）与 riskAdjustedFreeCapacity/riskAdjustedReceiverHeadroom 分立；旧名称 deprecated 兼容标注（projectedFreeCapacity→risk-adjusted、receiver projected headroom 语义标注）；receiver admission 走 risk-adjusted
- [ ] 14.12 指标与性能：authorization issued/rejected/invalidated/active、adapter mismatch、revision mismatch、intent count/slots/recoveries/conversions/store healthy/write failures、resolution in-progress/faulted/recovered、receipt refreshes、capability issued/rejected、reservation store healthy、risk-adjusted lookups；operation-count fixture（authorize/prepare 不全扫、intent admission O(1)、token 验证 O(1)）；容量模型入 design/evidence（receipt/intent/quarantine/resolution/authorization heap 上限与满载 fail-closed/恢复条件）
- [ ] 14.13 架构边界扩展：生产 writer 候选禁 executePreparedAction 任意 callback/compat/直接 prepareTransaction/自行构造 postings（唯一生产入口 = authorization→contract→注册 adapter）；adapter 注册仅 actionContracts.ts+测试；capability/reconciler 注册边界；intent store 直写仅 intents.ts
