...
- [ ] 14.14 验证与预算：typecheck/build/treasury 定向/全量 Jest/verify-jest-budget（新预算锚点，requiredBaselineCommit 指向含全部实现与测试的前置 commit）；evidence/round8-durable-intent-authorization-local-validation.md（起始 HEAD/最终 commit/状态机/覆盖与不能覆盖的 failure model/新增 Memory 字段与上限/命令/结果/未部署声明）；不部署、不接任何真实 Game writer
