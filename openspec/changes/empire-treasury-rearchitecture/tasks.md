...
- [ ] 15.1 安全 canonical contract encoding（canonicalEncoding.ts）：canonicalize（长度前缀编码 + key 排序确定 + 数组保序）先于 validate/derivePostings/execute（三者观察同一 canonical frozen args）；拒绝 cyclic/getter/setter/自定义 prototype/class instance/function/symbol/bigint/undefined/NaN/±Infinity/稀疏数组/非普通对象（结构化拒绝零抛出；getter 不被多次副作用读取）；{a:undefined}≠{}、NaN≠null 无静默碰撞；深度与编码长度有界；digest = AC2 前缀绑定 encoding version + action kind + adapter version + transactionId + canonical args + canonical postings + 结构身份；调用方构建后修改原 args 不影响 postings/digest/执行参数
- [ ] 15.2 adapter version binding + 完整结构 incarnation 验证：adapter 接口加 version 强绑定（contract 构建/授权 token/执行/capability 四处一致校验）；structureIds 任意字符串接口替换为受控 structureBindings(args)→{roomName, locationKind, label?}[]；contract 快照覆盖 posting locations + 全部声明结构；执行前逐结构重新验证（disappeared/replaced/kind mismatch → callback 零调用、授权零消费、tentative 不变）；fresh observation 必需——配额耗尽拒绝执行（不退回 shared）
- [ ] 15.3 contract-first authorization：authorizeTreasuryActionContract（contract 驱动；actionKind/rooms/locations/每资源 amount/contractDigest 全部从 canonical postings 派生；contractDigest 不可选；token 绑定 adapter version）；授权要求 write admission ready；自由字符串 policyFingerprint 通道移除（policy authority 仅受控 withhold 数值）；authorizeResourceUse 降级 @internal test-only（架构测试守护）；原子签发 bundle（任一资源失败时已签发 token 预算回滚）
- [ ] 15.4 原子 bundle redemption：executeTreasuryActionContract 全量只读预验证（对象身份/generation/tick/revisions/contract identity+digest/action kind/adapter version/transactionId/重复 token/exact posting coverage/每资源累计 amount/receiver capacity/policy）→ 结构校验（fresh）→ prepare（tentative 接管）→ 经 execution options 的 redeem hook 在 callback 前一次性消费 → intent → callback；任一失败零消费、budget 不变、callback 零调用；prepare 拒绝不留"授权已消费但 tentative 未接管"窗口
- [ ] 15.5 writer kernel 封闭：executePreparedAction/prepareTransaction/consumeTreasuryAuthorization/authorizeResourceUse 从公共生产面收敛（service 对象保留 @internal；execution options 窄接口供 actionContracts）；架构测试升级为扫描全部生产 TypeScript 源码（新增模块自动受约束；白名单仅 treasury 协议实现文件）
- [ ] 15.6 intent 完整合同身份（intents.ts v2）：entry 绑定 contractId/contractDigest/actionKind/adapterVersion/authorizationDigest/结构 incarnation/有界 durable payload（adapter.durableFacts 版本化 ≤512，不持久化完整 args/observation）；v1→v2 无损升级 + load 全量验证 + runtime.d.ts 同步
- [ ] 15.7 严格 phase 状态机：transitionTreasuryIntentPhase 验证期望前序 + digest/contract 一致（幂等仅限同 identity 且已处目标 phase）；合法迁移表（ready→executing→{returned_non_ok|ok_pending_commit}→execution_unknown 升级；quarantined/resolution_pending 受限进入；committed/aborted 终态）；not_found/store fatal/非法前序/identity mismatch 一律 callback 零调用（修复 not_found 被忽略的缺陷）；写入后 read-back 验证；callback 后 phase 写失败不普通 commit/abort（OK→executed_unsettled、非 OK→executed_abort_failed、抛错→execution_unknown），全部失败路径检查并处置 mark 结果
- [ ] 15.8 recovery 事实等级：ready→确认未执行释放；executing/execution_unknown→execution-unknown quarantine；returned_non_ok→action_returned_non_ok_abort_failed（保留"Game 已返回非 OK"）；ok_pending_commit→新增 commit 类 phase ok_pending_commit_unresolved（永不 not-executed resolution——事实单调性）；committed/aborted 终态残留幂等释放；emergency intent 保留原 phase
