...
1. 先完成独立代码审查及本任务包 G1/FC1 成本结果的评审，并为**本候选正式上传与一次 writer 灰度**取得单独决定。此前仅部署过巡逻修复；本候选尚未上传。G1 原包的恢复 `main` 是旧字节，必须重建与当前巡逻基线一致的候选/恢复身份。
...
本文件是可审查的首次灰度操作边界，不是已获正式部署授权或线上成功记录。G2 在隔离引擎的成功链路与 CPU 观察已完成；正式 shard1 灰度、独立审查和 G1 四点实测仍待执行。
