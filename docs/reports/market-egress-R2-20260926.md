...
> 本文保留上线前的 12:35 UTC 基线与离线验收。随后用户直接批准的一次 shard1 灰度已执行；实际代码上传、cfg/permit 迁移、0 次 native 的容量恢复退出与原始证据见 [正式试运行报告](market-egress-R2-live-trial-20260926.md)。下文“尚未上传”只描述该历史时点。

## 当前结论

截至 2026-09-26 12:35 UTC，本轮新代码仅作为 Git 分支候选；`shard1/default` 仍运行 r6 `2026.8.29-6+529245b@2026-09-26T08:07:39.599Z`。本轮未上传、未迁移 cfg/permit、未启动加速实例。原 r6 已在上一报告的 tick `73953712` 之后自主完成三次后继成交：tick `73954752` 的 E4N58:X 1,000，以及 tick `73955752`、`73957767` 的 E1N57:L 各 1,000，均写入 V3 `confirmed` receipt。12:23 UTC tick `73957705` 时旧冷却已过但当时无选单；后继第 15 笔在 62 tick 后完成，说明不能把一次无选单视为持续程序故障。12:35 UTC tick `73957895` 的 V3 滚动确认量为 8,000，pending/quarantine 均为零，当前旧冷却 `notBefore=73958767`。
...
| 15 | 73957767 | E1N57:L | 1,000 | 719 | 242.261009 | `6ab7ba21a2ce580013e7d923` |

四笔 receipt 所列经济净价合计 1,333,254.257 credits、手续费共 3,087 Energy；净价含 Energy 影子价格折算，**不是**游戏账户的实际 credits 入账；账户净变动还混有其他交易，本快照没有逐笔实际 credits 对账数据。上表的 13–15 是自主后继的 V3 receipt 与交易 ID；本轮没有通过另一个完整 outgoing 窗口独立重读这三笔。
...
- `market-egress-R2-evidence-20260926/r6-ledger-20260926T1234Z.json` 与 `r6-monitor-after15.json`：第 15 笔后的原始 V3 账本与 tick `73957895` 巡逻快照。
