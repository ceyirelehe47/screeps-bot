/**
 * Core Rewrite III——R1-R8 基线反例（治愈复验版）。
 *
 * 基线红灯证据（13 failed / 4 对照 passed）见 evidence 的 core-rewrite-iii
 * baseline 目录（383ffc1 worktree 运行，文件名不含 .test. 不进默认收集，
 * 用 --runTestsByPath 显式运行）。本副本在修复后代码上复验：全部缺陷
 * 反例转绿（fixture schema version 升 3；断言不变）。
 */
...
describe("R2 own-reservation 全链一致（R03/R2）", () => {
  ...
  // 治愈复验版修正：holderId 须为已注册逻辑命名空间（synthesis:），使
  // commitments 分类为 typed logical-service owner——与 excludeOwner 的
  // identity key 匹配（基线版用未注册字符串导致分类 legacy-unresolved，
  // 排除语义本身不可达；正式行为测试见 C03）。
  it("exact owner 自己的预留 300：owner 流出 950 必须获准（上层排除后 1000 ≥ 950）", () => {
    ...
    service.beginTick();
    installOwnReservation("W1N57", "synthesis:W1N57:energy", 300);
    const admission = service.authorizeTreasuryActionContract(
      ...
      {
        workKey: "biz:r2:own",
        owner: { scope: "production-reservation", ownerKind: "logical-service", ownerId: "synthesis:W1N57:energy", roomName: "W1N57" },
      },
    );
