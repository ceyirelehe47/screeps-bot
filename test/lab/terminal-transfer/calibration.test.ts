...
// 本轮（Treasury Terminal Integration I · Continuation 0002）真实 facts；
// 历史 ti10001 / ec0001 文件保留为归档，不再与活配置配对。
const FACTS_TI10002 = join(__dirname, "fixtures", "calibration-facts-ti10002.json");
...
describe("Terminal Transfer Lab Run I · Calibration Rerun——独立配置核对（C02）", () => {
  ...
  it("场景 G CLI 只读与真实命令：本轮真实 facts 退出 0、三项不一致 fixture 退出 1 且三项齐报、坏输入退出 2；运行前后输入文件字节不变", () => {
    const watched = [FACTS_TI10002, FACTS_TI10001, FACTS_EC0001, FACTS_HEALTHY, FACTS_MISMATCH, join(__dirname, "labConfig.ts"), join(__dirname, "example.experiment.json")];
    const before = watched.map(sha256Of);
    // 健康对照使用当前编译配置匹配的本轮（Continuation 0002）真实 facts 副本：
    // 由本轮实机只读 control probe console 流 + 真实暂停点/管理时刻装配
    // （证据归档于 continuation-0002/control-and-validation/）；历史
    // ti10001/ec0001 与 cal-0002 healthy fixture 的配对核对分别由上一轮证据
    // 与场景 B–F 覆盖，不在此重复。
    const healthy = spawnSync(process.execPath, [CLI, "--facts", FACTS_TI10002], { encoding: "utf8" });
