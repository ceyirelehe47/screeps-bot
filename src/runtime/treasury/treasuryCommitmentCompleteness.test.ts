...
import { treasuryTestHarness, resetTreasuryCoreStoreForTest } from "@/runtime/treasury/testHarness";
...
describe("第六轮 authorizationSafe 联合判定与 blockers", () => {
  ...
  it("kernel store 损坏（unhealthy fail closed）：authorizationSafe=false，blockers 指示，数值保留", () => {
    const service = makeService();
    service.beginTick();
    // 【Core Rewrite I】旧 write-fault/quarantine store 已退役；等价阻断：
    // treasuryCore 活跃记录损坏（unhealthy ≠ absent，R2/A05 语义）。
    if (!Memory.runtime) Memory.runtime = {} as never;
    Memory.runtime.treasuryCore = {
      version: 3, installEpochId: "0123456789abcdef",
    ...
    const view = service.query({ resource: RESOURCE_ENERGY, rooms: ["W1N57"] });
    expect(view.authorizationSafe).toBe(false);
    expect(view.authorizationBlockers).toContain("kernel_store_unhealthy");
    expect(view.observed).toBe(120_000); // 数值保留供观察（不以归零掩盖原因）
    expect(view.projected).toBe(120_000);
  });

  it("kernel store 版本不兼容（incompatible ≠ unhealthy ≠ absent）：authorizationSafe=false", () => {
    const service = makeService();
    service.beginTick();
    // 未知版本 fail closed——不自动清库、不静默当空库。
    if (!Memory.runtime) Memory.runtime = {} as never;
    Memory.runtime.treasuryCore = { version: 99 } as never;
    const view = service.query({ resource: RESOURCE_ENERGY, rooms: ["W1N57"] });
    expect(view.authorizationSafe).toBe(false);
    expect(view.authorizationBlockers).toContain("kernel_store_incompatible");
  });

  it("旧业务数据存在（legacy store present）：authorizationSafe=false 且 blockers 指示", () => {
    const service = makeService();
    service.beginTick();
        if (!Memory.runtime) Memory.runtime = {} as never;
    ...
    const view = service.query({ resource: RESOURCE_ENERGY, rooms: ["W1N57"] });
    expect(view.authorizationSafe).toBe(false);
    expect(view.authorizationBlockers).toContain("legacy_store_present");
