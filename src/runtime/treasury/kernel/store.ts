...
function validateWorkRecord(attemptId: string, value: unknown): string | null {
  ...
  if (r.outcomeEvidence !== null) {
    ...
    if (!boundedString(evidence.source, TREASURY_CORE_EVIDENCE_SOURCE_MAX)) {
...
function validateSafetyCore(root: Record<string, unknown>): string | null {
  ...
  if (((recovery as Record<string, unknown>).budgetUsed as number) > TREASURY_CORE_RECOVERY_BUDGET_PER_TICK_LIMIT()) {
    return "recovery.budgetUsed 非法（超预算记账）";
  }
...
/** recovery.budgetUsed 的合法上限（游标可大于 active 数，预算不行）。 */
function TREASURY_CORE_RECOVERY_BUDGET_PER_TICK_LIMIT(): number {
  return 1_000_000;
}
...
export function initializeTreasuryCoreStore(nowTick: number): { initialized: boolean; reason?: string } {
  ...
  if (readBack.status !== "healthy" || !treasuryCoreStateEquals(readBack.memory, fresh)) {
    delete runtime.treasuryCore;
    return {
      initialized: false,
      reason: `初始化读回失败：${readBack.status === "absent" ? "缺失" : readBack.status === "healthy" ? "读回与安装目标不一致" : readBack.reason}`,
    };
  }
...
/**
 * 受控写入 + 发布确认（R02）。流程：
 * 1. 快照基线（clone）并在草稿上执行 mutate；
 * 2. 基线漂移检查：写入前 Memory 当前值必须与基线一致（重入导致目标
 *    基线变化时拒绝陈旧草稿覆盖已推进状态）；
 * 3. 写入（根对象整体替换）；
 * 4. 读回与草稿深度精确比较 + 安全校验：读回是合法旧值 / 单字段未更新 /
 *    写被丢弃 / 安全层损坏一律视为失败并回滚；
 * 5. 失败回滚到基线快照（发布未发生语义）。
 */
export function writeTreasuryCoreMemory(
  ...
  const exact = treasuryCoreStateEquals(readback, draft);
  const safety = exact ? validateSafetyCore(readback as Record<string, unknown>) : "读回与目标安全状态不一致";
  if (!exact || safety !== null) {
    if (baseline === undefined) delete runtime.treasuryCore;
    else runtime.treasuryCore = baseline;
    onRollback(baseline);
    return {
      status: "failed",
      reason: `发布确认失败：${!exact ? "读回与预期目标安全快照不一致（丢写/旧值/字段未更新）" : safety}`,
    };
  }
...
/** 当前序列化体积（字符数 = JSON.stringify 长度；ASCII 布局下同 UTF-8 bytes）。 */
