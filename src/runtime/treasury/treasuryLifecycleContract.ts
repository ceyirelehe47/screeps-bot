...
export const TREASURY_STORE_LIFECYCLE_CONTRACTS: readonly TreasuryStoreLifecycleContract[] = [
  ...
  },
  {
    ...
    terminalCondition: "chain summary 持久化后孤儿 proof 释放（tombstone 不存在的代）；满载驱逐需 exact replacement relation 全维度通过且 exact 依赖（journal/tombstone/active lineage）关闭",
    cleanupOwner: "generationRetirementAuthority.ts + lineageRetirementSummary.ts 压缩；tombstone 退休联动释放",
    replacementAuthority: "retirement summary（exact replacement relation——root ID/lineage/issuer domain/terminalState 相容/authorityClass/identity profile/digest/canonical root identity/durable/contract/cohort/lowlevel 矩阵全维度；legacy replay-only summary 不授权）",
    retentionPolicy: "queue-evict-eligible",
    overflowBehavior: "fail-closed",
  ...
  },
  {
    ...
    protectedFact: "已退休发行序号区间（同一发行域内相邻单调合并——ti1_(legacy) 与 ti2_(current) 的同序号是两个独立发行事实，X 工作流 D：按 namespace 隔离；【XII/E】v3 起两域为同一版本化 store 内的物理独立分区）",
    hardCapacity: 112,
    capacityNote: "【XII/E】v3 物理分区：current 分区硬上限 48（TREASURY_RETIRED_RANGE_CURRENT_CAPACITY）/ legacy 分区配额 16（TREASURY_RETIRED_RANGE_LEGACY_QUOTA）、迁移存量显式 legacyOverflow 至多 64（TREASURY_RETIRED_RANGE_LEGACY_CAPACITY）——总 Memory 上限 112（TREASURY_RETIRED_RANGE_MAX_ENTRIES）；legacy 占满旧协议最大 64 条时 current 仍有完整 48 条保留容量；某分区 overflow 只阻断该分区新吸收（legacyOverflow 下 legacy 新增区间一律 fail closed）；不删除旧安全事实腾槽、不跨域合并；current 分区孤儿 gap coalesce 收敛（legacy gap 不参与——canonical ID 不可重建）",
    terminalCondition: "区间永久（anti-reuse 不退出——只合并不删除）",
    cleanupOwner: "chainRetirementCertificate.ts absorb/coalesce（分区单调合并 + 分区容量终检；absorb 必须携带 issuer domain）；【XI/E → XII/E】v1/v2 → v3 分区迁移唯一 owner = treasuryLifecycleGcCoordinator 的 tick-boundary migration 阶段（源形状校验 + 发行域严格证明/namespace 分流 + 单对象替换 + read-back + 区间数守恒；不可证明 → blocked 原数据保留）",
    replacementAuthority: null,
    retentionPolicy: "none",
    overflowBehavior: "compact",
    resetRecovery: "Memory 直读（v3 分区 shape 校验：分区 namespace 一致 + 域内严格递增 + legacyOverflow 与超额事实一致；v1/v2 store 经 tick-boundary migration owner 显式迁移——发行域可从版本边界严格证明则归对应分区（v2 按 namespace 标签分流），否则 forensic fail closed，不静默猜测、不静默裁剪；迁移幂等——reset 后重读 v3 即完成，不产生第二 frontier）",
    lookupBound: "O(分区区间数) 扫描（按 ID 自带 namespace 选分区；query 零写——v1/v2 报告 migration_required，不迁移不初始化空店）",
    gcBound: "current 分区孤儿 gap coalesce ≤512 序号 × 生命周期权威探测（namespace-local）",
    allowsAgeEviction: false,
  },
  {
  ...
  },
  {
    ...
    hardCapacity: 128,
    capacityNote: "TREASURY_ISSUED_TICKET_MAX_TOTAL_ENTRIES=128（总 entry 含 terminal——shape validator 强制，B1）；active 并发上限 64；TTL=500",
    terminalCondition: "consumed（opening 接管）/ expired（TTL 显式转换——正面生命周期事实）",
    cleanupOwner: "treasuryLifecycleGcCoordinator.ts（expire + terminal retire，watermark frontier 验证；open 满载时先行 bounded reclaim）",
    ...
    lookupBound: "O(1) 单键",
    gcBound: "expire/retire/GC 全表扫描 ≤ 总硬容量 128；retire 每批 ≤8",
    allowsAgeEviction: false,
  },
  {
    storeKey: "lifecycle",
