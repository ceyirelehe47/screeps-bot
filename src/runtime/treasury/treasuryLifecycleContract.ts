...
export const TREASURY_STORE_LIFECYCLE_CONTRACTS: readonly TreasuryStoreLifecycleContract[] = [
  ...
  },
  {
    ...
    hardCapacity: 64,
    capacityNote: "TREASURY_RETIRED_RANGE_MAX_ENTRIES=64 总区间；【XI/F】per-namespace 配额 current=48（TREASURY_RETIRED_RANGE_CURRENT_QUOTA）/ legacy=16（TREASURY_RETIRED_RANGE_LEGACY_QUOTA）——legacy 满配额不耗尽 current 保留容量；超额 legacy 存量保留不裁剪（只阻断新增区间的吸收）；两域互不驱逐、不跨域合并；current 域孤儿 gap coalesce 收敛（legacy gap 不参与——canonical ID 不可重建）",
    terminalCondition: "区间永久（anti-reuse 不退出——只合并不删除）",
    cleanupOwner: "chainRetirementCertificate.ts absorb/coalesce（同域单调合并 + 域内 quota 终检；absorb 必须携带 issuer domain）；【XI/E】v1→v2 迁移唯一 owner = treasuryLifecycleGcCoordinator 的 tick-boundary migration 阶段（源形状校验 + 发行域严格证明 + 单对象替换 + read-back；不可证明 → blocked 原数据保留）",
    replacementAuthority: null,
    retentionPolicy: "none",
    overflowBehavior: "compact",
    resetRecovery: "Memory 直读（v2 shape 校验：namespace 枚举 + 同域严格递增；v1 裸 sequence store 经 tick-boundary migration owner 显式迁移——发行域可从版本边界严格证明则归对应域，否则 forensic fail closed，不静默猜测；迁移幂等——reset 后重读 v2 即完成，不产生第二 frontier）",
    lookupBound: "O(64) 区间扫描（按 ID 自带 namespace 过滤；query 零写——v1 报告 migration_required，不迁移不初始化空店）",
