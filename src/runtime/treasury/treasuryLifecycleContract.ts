...
export const TREASURY_STORE_LIFECYCLE_CONTRACTS: readonly TreasuryStoreLifecycleContract[] = [
  ...
  },
  {
    ...
    protectedFact: "已退休发行序号区间（相邻单调合并——O(1) anti-reuse frontier）",
    hardCapacity: 64,
    capacityNote: "TREASURY_RETIRED_RANGE_MAX_ENTRIES=64 区间；孤儿 gap coalesce 收敛",
    terminalCondition: "区间永久（anti-reuse 不退出——只合并不删除）",
    cleanupOwner: "chainRetirementCertificate.ts absorb/coalesce（单调合并）",
    replacementAuthority: null,
    retentionPolicy: "none",
    overflowBehavior: "compact",
    resetRecovery: "Memory 直读（v1 shape 校验；区间严格递增）",
    lookupBound: "O(64) 区间扫描",
    gcBound: "孤儿 gap coalesce ≤512 序号 × 生命周期权威探测",
    allowsAgeEviction: false,
  },
  {
