...
interface ReservationRuntimeMetadata {
  resourceReservations?: ReservationStore;
  resourceReservationsOwnerVersion?: number;
  resourceReservationsCorrupted?: string;
}

function reservationRuntime(): ReservationRuntimeMetadata | undefined {
  return Memory.runtime as unknown as ReservationRuntimeMetadata | undefined;
}
...
type ReservationAdapterMode = "legacy" | "typed" | "blocked";

/**
 * Keep the deployed reservation writer byte-for-byte in behavior while Treasury
 * is OFF. Once schema v4 has been activated, legacy callers stay on the typed
 * API so they can never reintroduce old keys into a migrated store.
 */
function reservationAdapterMode(): ReservationAdapterMode {
  const runtime = Memory.runtime as (NonNullable<typeof Memory.runtime> & { resourceReservationsOwnerVersion?: unknown }) | undefined;
  const version = runtime?.resourceReservationsOwnerVersion;
  const rawConfig = (Memory as unknown as { cfg?: Record<string, unknown> }).cfg?.treasuryTerminalTransferSlice0;
  const mode = rawConfig && typeof rawConfig === "object" && !Array.isArray(rawConfig)
    ? (rawConfig as Record<string, unknown>).mode
    : undefined;
  if (version === RESERVATION_OWNER_VERSION || mode === "canary" || mode === "drain") return "typed";
  if (version !== undefined && version !== 1 && version !== 2 && version !== 3) return "blocked";
  return "legacy";
}

function legacyMakeKey(roomName: string, resource: ResourceConstant, holderId: string): string {
...
function legacyReserve(
  ...
  if (amount <= 0) return { status: "ok", mutated: false };
  const store = ensureStore();
  store[legacyMakeKey(roomName, resource, holderId)] = {
    roomName,
    resource,
    holderId,
    amount,
...
function legacyRelease(roomName: string, resource: ResourceConstant, holderId: string): ReservationMutationResult {
  if (!Memory.runtime?.resourceReservations) return { status: "ok", mutated: false };
  const key = legacyMakeKey(roomName, resource, holderId);
  const mutated = Object.prototype.hasOwnProperty.call(Memory.runtime.resourceReservations, key);
  delete Memory.runtime.resourceReservations[key];
  return { status: "ok", mutated };
}

function legacyRenew(
  roomName: string,
  resource: ResourceConstant,
  amount: number,
  holderId: string,
  ...
  const store = ensureStore();
  const key = legacyMakeKey(roomName, resource, holderId);
  if (!store[key]) return { status: "ok", mutated: false };
  store[key] = { roomName, resource, holderId, amount, updatedAt: Game.time, expiresAt: Game.time + ttl };
  return { status: "ok", mutated: true };
}
...
export function ensureReservationSchemaActivated(): ReservationSchemaGate {
  const runtime = reservationRuntime();
  ...
  if (!store || Object.keys(store).length === 0) {
    ...
    reservationRuntime()!.resourceReservationsOwnerVersion = RESERVATION_OWNER_VERSION;
    bumpReservationStoreRevision();
    return { status: "ready" };
  }
  // v1 语义 = version 字段缺失（undefined）且 store 非空——宽化为 number
  // 绕过声明类型的字面量收窄（declared 只声明 2|3|4，缺省即 v1）。
  const declared = reservationRuntime()!.resourceReservationsOwnerVersion;
...
export function reserveProductionResource(
  ...
  const mode = reservationAdapterMode();
  if (mode === "legacy") return legacyReserve(roomName, resource, amount, holderId, ttl);
  if (mode === "blocked") return rejected("schema_not_ready", "unknown reservation owner version; refusing to mix legacy keys");
...
export function releaseProductionReservation(
  ...
  const mode = reservationAdapterMode();
  if (mode === "legacy") return legacyRelease(roomName, resource, holderId);
  if (mode === "blocked") return rejected("schema_not_ready", "unknown reservation owner version; refusing to mix legacy keys");
...
export function renewProductionReservation(
  ...
  const mode = reservationAdapterMode();
  if (mode === "legacy") return legacyRenew(roomName, resource, amount, holderId, ttl);
  if (mode === "blocked") return rejected("schema_not_ready", "unknown reservation owner version; refusing to mix legacy keys");
...
export function getReservedProductionAmount(roomName: string, resource: ResourceConstant): number {
  ...
  for (const entry of Object.values(store)) {
    if (reservationAdapterMode() !== "legacy" && !Number.isSafeInteger(entry.amount)) continue;
...
export function getReservedProductionAmountExcludingOwner(
  ...
  const store = reservationRuntime()?.resourceReservations;
  if (!store) return 0;
...
export function gcProductionReservations(): ReservationGcReport {
  ...
  if (reservationAdapterMode() === "legacy") {
  ...
  if (reservationAdapterMode() === "blocked") return report;
  let removed = 0;
  for (const [key, entry] of Object.entries(store)) {
...
export function listProductionReservations(): readonly Readonly<ReservationEntry>[] {
  ...
  if (reservationAdapterMode() === "legacy") return Object.values(store);
...
export function migrateResourceReservationsForTypedOwner(): ReservationOwnerMigrationReport {
  ...
  if (!Memory.runtime) Memory.runtime = {};
  const updatedRuntime = reservationRuntime()!;
  updatedRuntime.resourceReservations = rebuilt;
  updatedRuntime.resourceReservationsOwnerVersion = RESERVATION_OWNER_VERSION;
  bumpReservationStoreRevision();
